...
static u32 GetRematchIndex(u32 trainerIdx);
...
{
    u32 whichLeader = 0;
    u32 lowestRematchIndex = 5;
    u32 i;
    u32 rematchIndex;
    for (i = 0; i < size; i++)
    {
        if (!gSaveBlock1.trainerRematches[data[i]])
        {
            ...
            for (i = 0; i < size; i++)
            {
                if (!gSaveBlock1.trainerRematches[data[i]])
                {
                    ...
                    if (rematchIndex == lowestRematchIndex)
                    {
                        if (whichLeader == 0)
                        {
                            gSaveBlock1.trainerRematches[data[i]] = lowestRematchIndex;
                            break;
                        }
                        whichLeader--;
                    }
