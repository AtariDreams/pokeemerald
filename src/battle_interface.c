...
static void TextIntoHealthboxObject(void *, const u8 *, u32);
static void SafariTextIntoHealthboxObject(void *, const u8 *, u32);
static void HpTextIntoHealthboxObject(void *, const u8 *, u32);
...
static const struct SpriteSheet sStatusSummaryBarSpriteSheet =
{
...
u8 CreateBattlerHealthboxSprites(u8 battlerId)
{
    ...
    else
    {
        if (GetBattlerSide(battlerId) == B_SIDE_PLAYER)
        {
            healthboxLeftSpriteId = CreateSprite(&sHealthboxPlayerSpriteTemplates[GetBattlerPosition(battlerId) >> 1], DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
            healthboxRightSpriteId = CreateSpriteAtEnd(&sHealthboxPlayerSpriteTemplates[GetBattlerPosition(battlerId) >> 1], DISPLAY_WIDTH, DISPLAY_HEIGHT, 1);
    ...
    SetSubspriteTables(&gSprites[healthbarSpriteId], &sHealthBar_SubspriteTables[GetBattlerSide(battlerId)]);
    gSprites[healthbarSpriteId].subspriteMode = SUBSPRITES_IGNORE_PRIORITY;
    ...
    gSprites[healthbarSpriteId].hBar_HealthBoxSpriteId = healthboxLeftSpriteId;
    gSprites[healthbarSpriteId].hBar_Data6 = data6;
...
void UpdateHpTextInHealthbox(u8 healthboxSpriteId, s16 value, u8 maxOrCurrent)
{
    ...
    u8 *p;
    if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) == B_SIDE_PLAYER && !(gBattleTypeFlags & BATTLE_TYPE_DOUBLE))
    {
        u8 text[32];
        p = (u8 *)(OBJ_VRAM0) + gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;
        if (maxOrCurrent != HP_CURRENT) // singles, max
        {
            ConvertIntToDecimalStringN(text, value, STR_CONV_MODE_RIGHT_ALIGN, 3);
            windowTileData = AddTextPrinterAndCreateWindowOnHealthbox(text, 0, 5, 2, &windowId);
            HpTextIntoHealthboxObject(p + 90 * 32, windowTileData, 2);
            RemoveWindowOnHealthbox(windowId);
        }
        else // singles, current
        {
            ...
            HpTextIntoHealthboxObject(p + 31 * 32, windowTileData, 1);
            HpTextIntoHealthboxObject(p + 88 * 32, windowTileData + 0x20, 2);
            RemoveWindowOnHealthbox(windowId);
        }
    }
    else
    {
        ...
        if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || GetBattlerSide(battler) == B_SIDE_OPPONENT)
        {
        ...
        {
            u8 text[20] = {EXT_CTRL_CODE_BEGIN, CHAR_A_GRAVE, 1, EXT_CTRL_CODE_BEGIN, CHAR_A_ACUTE, 0x2};
            u32 var;
            u32 i;
            ...
            for (i = 0; i < 3; i++)
            {
                CpuFastCopy(&gMonSpritesGfxPtr->barFontGfx[i * 64 + 32],
...
static void UpdateHpTextInHealthboxInDoubles(u8 healthboxSpriteId, s16 value, u8 maxOrCurrent)
{
    ...
    if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) == B_SIDE_PLAYER)
    {
            ...
            return;
        {
            ...
            p = (u8 *)(OBJ_VRAM0) + gSprites[gSprites[healthboxSpriteId].data[5]].oam.tileNum * TILE_SIZE_4BPP;

            if (maxOrCurrent != HP_CURRENT) // doubles, max hp
            {
                ConvertIntToDecimalStringN(text, value, STR_CONV_MODE_RIGHT_ALIGN, 3);
                windowTileData = AddTextPrinterAndCreateWindowOnHealthbox(text, 0, 5, 0, &windowId);
                HpTextIntoHealthboxObject(p + 0xC0, windowTileData, 2);
                RemoveWindowOnHealthbox(windowId);
            ...
            else
            {
                ...
                FillHealthboxObject(p, 0, 3); // Erases HP bar leftover.
                HpTextIntoHealthboxObject(p + 3 * 32, windowTileData, 3);
                RemoveWindowOnHealthbox(windowId);
            }
        }
    }
    else
    {
        ...
        if (!gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars) // don't print text if only bars are visible
            return;
        {
            u8 text[20] = {EXT_CTRL_CODE_BEGIN, CHAR_A_GRAVE, 1, EXT_CTRL_CODE_BEGIN, CHAR_A_ACUTE, 0};
            u8 var;
            u8 r7;
            u8 *txtPtr;
            u8 i;
            ...
            for (i = var; i < var + 3; i++)
            {
                if (i < 3)
                {
                    CpuFastCopy(&gMonSpritesGfxPtr->barFontGfx[((i - var) * 64) + 32],
                          (void *)((OBJ_VRAM0 + 0x20) + 32 * (gSprites[r7].oam.tileNum + i)),
                          0x20);
                }
                else
                {
            ...
            if (maxOrCurrent == HP_CURRENT)
            {
                CpuFastCopy(&gMonSpritesGfxPtr->barFontGfx[224],
                          (void *)((OBJ_VRAM0) + ((gSprites[r7].oam.tileNum + 4) * TILE_SIZE_4BPP)),
                          0x20);
                CpuFastFill(0, (void *)((OBJ_VRAM0) + (gSprites[r7].oam.tileNum * TILE_SIZE_4BPP)), 0x20);
            }
            else
            {
                if (GetBattlerSide(battlerId) == B_SIDE_PLAYER) // Impossible to reach part, because the battlerId is from the opponent's side.
                {
                    CpuFastCopy(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_FRAME_END),
...
static void PrintSafariMonInfo(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    for (j = 6, i = 0; i < var; i++, j++)
    {
        ...
        CpuFastCopy(GetHealthboxElementGfxPtr(elementId), barFontGfx + (i * 64), 0x20);
    }

    for (j = 1; j < var + 1; j++)
    {
        spriteTileNum =  (u8 *)(OBJ_VRAM0) + (gSprites[healthboxSpriteId].oam.tileNum + ((j << 3) | (j & 7))) * TILE_SIZE_4BPP;
        CpuFastCopy(barFontGfx, spriteTileNum, 0x20);
        barFontGfx += 0x20;

        spriteTileNum = (u8 *)(OBJ_VRAM0) + (8 + gSprites[healthboxSpriteId].oam.tileNum + ((j << 3) | (j & 7))) * TILE_SIZE_4BPP;
        CpuFastCopy(barFontGfx, spriteTileNum, 0x20);
        barFontGfx += 0x20;
    }
    ...
    for (j = 0; j < 5; j++)
    {
        if (j < 2)
        {
            CpuFastCopy(&gMonSpritesGfxPtr->barFontGfx[0x40 * j + 0x20],
                      (void *)(OBJ_VRAM0 + 0x40) + (gSprites[healthBarSpriteId].oam.tileNum + j) * TILE_SIZE_4BPP,
                      32);
        }
        else
        {
...
void SwapHpBarsWithHpText(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (gSprites[gHealthboxSpriteIds[i]].callback == SpriteCallbackDummy
         ...
         && ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || GetBattlerSide(i) != B_SIDE_PLAYER))
        {
            bool8 noBars;
            ...
            if (GetBattlerSide(i) == B_SIDE_PLAYER)
            {
                ...
                else // text to bars
                {
                    ...
                    CpuFastCopy(GetHealthboxElementGfxPtr(HEALTHBOX_GFX_FRAME_END_BAR), (void *)(OBJ_VRAM0 + 0x680 + gSprites[gHealthboxSpriteIds[i]].oam.tileNum * TILE_SIZE_4BPP), 32);
                }
            }
            else
            {
                if (noBars) // bars to text
                {
                    ...
                    else
                    {
                        ...
                        CpuFastCopy(0, (void *)(OBJ_VRAM0 + gSprites[healthBarSpriteId].oam.tileNum * 32), 0x100);
...
u8 CreatePartyStatusSummarySprites(u8 battlerId, struct HpAndStatus *partyInfo, bool8 skipPlayer, bool8 isBattleStart)
{
    ...
    if (!skipPlayer || GetBattlerPosition(battlerId) != B_POSITION_OPPONENT_RIGHT)
    {
        ...
        else
        {
            ...
            if (!skipPlayer || !(gBattleTypeFlags & BATTLE_TYPE_DOUBLE))
                bar_X = 104, bar_Y = 40;
            else
                bar_X = 104, bar_Y = 16;
...
static void UpdateNickInHealthbox(u8 healthboxSpriteId, struct Pokemon *mon)
{
    ...
    spriteTileNum = (u8 *)OBJ_VRAM0 + gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;

    if (GetBattlerSide(gSprites[healthboxSpriteId].data[6]) == B_SIDE_PLAYER)
    {
        TextIntoHealthboxObject((void *)(spriteTileNum + 0x40), windowTileData, 6);
        if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE))
            spriteTileNum += 0x800;
        else
            spriteTileNum += 0x400;
    ...
    {
        TextIntoHealthboxObject(spriteTileNum + 32, windowTileData, 7);
    }
...
static void UpdateStatusIconInHealthbox(u8 healthboxSpriteId)
{
    ...
    if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || GetBattlerSide(battlerId) == B_SIDE_OPPONENT)
    {
        if (!gBattleSpritesDataPtr->battlerData[battlerId].hpNumbersNoBars)
        {
...
static void UpdateSafariBallsTextOnHealthbox(u8 healthboxSpriteId)
{
    ...
    spriteTileNum = (u8*)OBJ_VRAM0 + gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;
    TextIntoHealthboxObject(spriteTileNum + 0x40, windowTileData, 6);
    TextIntoHealthboxObject(spriteTileNum + 0x800, windowTileData + 0xC0, 2);
    RemoveWindowOnHealthbox(windowId);
}
...
static void UpdateLeftNoOfBallsTextOnHealthbox(u8 healthboxSpriteId)
{
    ...
    spriteTileNum = (u8 *)OBJ_VRAM0 + gSprites[healthboxSpriteId].oam.tileNum * TILE_SIZE_4BPP;
    SafariTextIntoHealthboxObject(spriteTileNum + 0x2C0, windowTileData, 2);
    SafariTextIntoHealthboxObject(spriteTileNum + 0xA00, windowTileData + 0x40, 4);
    RemoveWindowOnHealthbox(windowId);
}
...
void UpdateHealthboxAttribute(u8 healthboxSpriteId, struct Pokemon *mon, u8 elementId)
{
    ...
    // if (elementId == HEALTHBOX_ALL && !(gBattleTypeFlags & BATTLE_TYPE_DOUBLE))
    ...
    if (GetBattlerSide(gSprites[healthboxSpriteId].hMain_Battler) == B_SIDE_PLAYER)
    {
        ...
        if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE) && (elementId == HEALTHBOX_EXP_BAR || elementId == HEALTHBOX_ALL))
        {
            u16 species;
            s32 exp, maxExpBarValue;
            u8 level;

            LoadBattleBarGfx(3);
            species = GetMonData(mon, MON_DATA_SPECIES);
            level = GetMonData(mon, MON_DATA_LEVEL);
            exp = GetMonData(mon, MON_DATA_EXP) - gExperienceTables[gSpeciesInfo[species].growthRate][level];
            maxExpBarValue = gExperienceTables[gSpeciesInfo[species].growthRate][level + 1] - gExperienceTables[gSpeciesInfo[species].growthRate][level];
            SetBattleBarStruct(battlerId, healthboxSpriteId, maxExpBarValue, exp, 0);
...
{
    CpuFastFill(0x11111111 * valMult, dest, numTiles * TILE_SIZE_4BPP);
}

static void HpTextIntoHealthboxObject(void *dest, const u8 *windowTileData, u32 windowWidth)
{
    CpuFastCopy(windowTileData + 256, dest, windowWidth * TILE_SIZE_4BPP);
}

static void TextIntoHealthboxObject(void *dest, const u8 *windowTileData, u32 windowWidth)
{
    CpuFastCopy(windowTileData + 256, dest + 256, windowWidth * TILE_SIZE_4BPP);
// + 256 as that prevents the top 4 blank rows of sHealthboxWindowTemplate from being copied

    for (u32 i = 0; i < windowWidth; i++) {
...
{
    CpuFastCopy(windowTileData, dest, windowWidth * TILE_SIZE_4BPP);
    CpuFastCopy(windowTileData + 256, dest + 256, windowWidth * TILE_SIZE_4BPP);
}
