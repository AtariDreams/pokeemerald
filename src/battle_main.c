...
EWRAM_DATA u16 *gBattleAnimBgTilemapBuffer = NULL;
...
{
    InitHeap(); // Needs to be here to avoid glitches. TODO: revisit this
...
static void UNUSED HBlankCB_Battle(void)
{
    ...
    if (VCOUNT >= 111 && VCOUNT <= DISPLAY_HEIGHT)
...
static void SpriteCB_AnimFaintOpponent(struct Sprite *sprite)
{
    ...
    if (--sprite->data[4] == 0)
    {
        ...
        {
            u8 *dst = gMonSpritesGfxPtr->sprites.byte[GetBattlerPosition(sprite->sBattler)] + (gBattleMonForms[sprite->sBattler] *0x800) + (sprite->data[3] * 0x100);
...
static void BattleStartClearSetData(void)
{
    ...
    else if (GetBattleSceneInRecordedBattle())
        gHitMarker |= HITMARKER_NO_ANIMATIONS;
...
void SwitchInClearSetData(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        if (gBattleStruct->wrappedBy[i] == gActiveBattler)
...
void FaintClearSetData(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (gDisableStructs[i].battlerPreventingEscape == gActiveBattler)
            gBattleMons[i].status2 &= ~STATUS2_ESCAPE_PREVENTION;
        gBattleMons[i].status2 &= ~STATUS2_INFATUATED_WITH(gActiveBattler);
        if (gBattleStruct->wrappedBy[i] == gActiveBattler)
            gBattleMons[i].status2 &= ~STATUS2_WRAPPED;
...
void SwitchPartyOrder(u8 battler)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
    {
        for (i = 0; i < ARRAY_COUNT(gBattlePartyCurrentOrder); i++)
        {
            ...
            gBattleStruct->battlerPartyOrders[BATTLE_PARTNER(battler)][i] = gBattlePartyCurrentOrder[i];
        }
    }
    else
    {
...
static void ReturnFromBattleToOverworld(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_ROAMER)
    {
        ...
        if ((gBattleOutcome == B_OUTCOME_WON) || gBattleOutcome == B_OUTCOME_CAUGHT) // Bug: When Roar is used by roamer, gBattleOutcome is B_OUTCOME_PLAYER_TELEPORTED (5).                                                                             // & with B_OUTCOME_WON (1) will return TRUE and deactivates the roamer.
            SetRoamerInactive();
    }
