...
static void DisplayPartyPokemonBarDetail(u8, const u8 *, u8, const PosData *);
...
static void SetPartyMonFieldSelectionActions(u8);
...
{
    if (GetMonData(&gPlayerParty.party[slot], MON_DATA_IS_EGG))
    {
...
static void DisplayPartyPokemonDataForChooseHalf(u8 slot)
{
    ...
    {
        u8 entries = GetMaxBattleEntries();
        for (i = 0; i < entries; i++)
        {
...
{
    switch (GetContestEntryEligibility(&gPlayerParty.party[slot]))
    {
...
{
    if (GetNumberOfRelearnableMoves(&gPlayerParty.party[slot]) == 0)
...
{
    if (GetMonData(&gPlayerParty.party[slot], MON_DATA_HELD_ITEM))
...
{
    struct Pokemon *currentPokemon = &gPlayerParty.party[slot];
...
{
    switch (CanMonLearnTMTutor(&gPlayerParty.party[slot], item, tutor))
    {
...
static void DisplayPartyPokemonDataForMultiBattle(u8 slot)
{
    ...
    else
    {
        ...
        DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0, &menuBox->infoRects->name);
...
{
    return &sPartyBgGfxTilemap[tileId * 32];
}
...
static void CreatePartyMonSprites(u8 slot)
{
    ...
    if (GetMonData(&gPlayerParty.party[slot], MON_DATA_SPECIES) == SPECIES_NONE)
        return;
    CreatePartyMonIconSprite(&gPlayerParty.party[slot], &sPartyMenuBoxes[slot], slot);
    CreatePartyMonHeldItemSprite(&gPlayerParty.party[slot], &sPartyMenuBoxes[slot]);
    CreatePartyMonPokeballSprite(&gPlayerParty.party[slot], &sPartyMenuBoxes[slot]);
    CreatePartyMonStatusSprite(&gPlayerParty.party[slot], &sPartyMenuBoxes[slot]);
}
...
void AnimatePartySlot(u8 slot, u8 animNum)
{
    ...
    case PARTY_SIZE: // Confirm
        ...
        PartyMenuStartSpriteAnim(sPartyMenuInternal->spriteIdConfirmPokeball, animNum);
        ScheduleBgCopyTilemapToVram(1);
        break;
    case PARTY_SIZE + 1: // Cancel
        ...
        PartyMenuStartSpriteAnim(sPartyMenuInternal->spriteIdCancelPokeball, animNum);
        ScheduleBgCopyTilemapToVram(1);
        break;
    default:
        if (GetMonData(&gPlayerParty.party[slot], MON_DATA_SPECIES) != SPECIES_NONE)
...
static u8 GetPartyBoxPaletteFlags(u8 slot, u8 animNum)
{
    ...
    if (GetMonData(&gPlayerParty.party[slot], MON_DATA_HP) == 0)
...
{
    if (GetMonData(&gPlayerParty.party[*slotPtr], MON_DATA_IS_EGG) == TRUE)
    {
        PlaySE(SE_FAILURE);
        return FALSE;
    }
...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_DOWN:
        ...
        *slotPtr = 0;
        ...
        if (*slotPtr == gPlayerParty.count - 1)
        {
        if (sPartyMenuInternal->chooseHalf)
                *slotPtr = PARTY_SIZE;
        else
        ...
        break;
    case MENU_DIR_RIGHT:
        if (gPlayerParty.count != 1 && *slotPtr == 0)
        {
...
static void UpdatePartySelectionDoubleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_UP:
        ...
        newSlot = GetNewSlotDoubleLayout(*slotPtr, MENU_DIR_UP);
        if (newSlot != -1)
            *slotPtr = newSlot;
        break;
    case MENU_DIR_DOWN:
    ...
    case MENU_DIR_RIGHT:
        if (*slotPtr == 0)
        {
            ...
            {
                if (GetMonData(&gPlayerParty.party[3], MON_DATA_SPECIES) != SPECIES_NONE)
                    *slotPtr = 3;
            }
            else if (GetMonData(&gPlayerParty.party[2], MON_DATA_SPECIES) != SPECIES_NONE)
            {
                *slotPtr = 2;
            }
            break;
        ...
        if (*slotPtr == 1)
        {
            ...
            {
                if (GetMonData(&gPlayerParty.party[5], MON_DATA_SPECIES) != SPECIES_NONE)
                    *slotPtr = 5;
            }
            else if (GetMonData(&gPlayerParty.party[4], MON_DATA_SPECIES) != SPECIES_NONE)
            {
                *slotPtr = 4;
            }
        }
...
static s8 GetNewSlotDoubleLayout(s8 slotId, s8 movementDir)
{
    ...
    {
        slotId += movementDir;
        if (slotId >= PARTY_SIZE || slotId < 0)
            return -1;
        if (GetMonData(&gPlayerParty.party[slotId], MON_DATA_SPECIES) != SPECIES_NONE)
            return slotId;
    }
}
...
static void Task_PartyMenuModifyHP(u8 taskId)
{
    ...
    tHP += tHPIncrement;
    tHPToAdd--;
    SetMonData(&gPlayerParty.party[tPartyId], MON_DATA_HP, &tHP);
    DisplayPartyPokemonHPCheck(&gPlayerParty.party[tPartyId], &sPartyMenuBoxes[tPartyId], 1);
    DisplayPartyPokemonHPBarCheck(&gPlayerParty.party[tPartyId], &sPartyMenuBoxes[tPartyId]);
        ...
        *ptr = 0;
        if (gSpecialVar_0x8005 == 0)
        {
            for (i = 0; i < gPlayerParty.count; i++)
                *ptr += IsMonAllowedInPokemonJump(&gPlayerParty.party[i]) << i;
        }
        else
        {
            for (i = 0; i < gPlayerParty.count; i++)
                *ptr += IsMonAllowedInDodrioBerryPicking(&gPlayerParty.party[i]) << i;
        }
    }
}
...
static void DisplayPartyPokemonBarDetail(u8 windowId, const u8 *str, u8 color, const PosData* pos)
{
    AddTextPrinterParameterized3(windowId, FONT_SMALL,  pos->px, pos->py, sFontColorTable[color], 0, str);
}
...
static void DisplayPartyPokemonNickname(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    {
        if (c == 1)
            menuBox->infoRects->blitFunc(menuBox->windowId, menuBox->infoRects->name.px / 8, menuBox->infoRects->name.py / 8, menuBox->infoRects->name.sx/8, menuBox->infoRects->name.sy/8, FALSE);
        GetMonNickname(mon, nickname);
        DisplayPartyPokemonBarDetail(menuBox->windowId, nickname, 0, &menuBox->infoRects->name);
    }
}
...
static void DisplayPartyPokemonLevelCheck(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE)
    {
        ...
        {
            if (c != 0)
                menuBox->infoRects->blitFunc(menuBox->windowId, menuBox->infoRects->level.px/8, (menuBox->infoRects->level.py/8) + 1, menuBox->infoRects->level.sx/8, menuBox->infoRects->level.sy /8, FALSE);
            if (c != 2)
...
static void DisplayPartyPokemonLevel(u8 level, struct PartyMenuBox *menuBox)
{
    ...
    DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0, &menuBox->infoRects->level);
}
...
static void DisplayPartyPokemonGenderNidoranCheck(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (c == 1)
        menuBox->infoRects->blitFunc(menuBox->windowId, menuBox->infoRects->gender.px/8, (menuBox->infoRects->gender.py/8) + 1, menuBox->infoRects->gender.sx/8, menuBox->infoRects->gender.sy/8, FALSE);
    GetMonNickname(mon, nickname);
...
static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname, struct PartyMenuBox *menuBox)
{
    ...
    case MON_MALE:
        ...
        DisplayPartyPokemonBarDetail(menuBox->windowId, gText_MaleSymbol, 2, &menuBox->infoRects->gender);
        break;
    case MON_FEMALE:
        ...
        DisplayPartyPokemonBarDetail(menuBox->windowId, gText_FemaleSymbol, 2, &menuBox->infoRects->gender);
        break;
    }
}
...
static void DisplayPartyPokemonHPCheck(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    {
        if (c != 0)
            menuBox->infoRects->blitFunc(menuBox->windowId, menuBox->infoRects->HP.px/8, (menuBox->infoRects->HP.py/8) + 1, menuBox->infoRects->HP.sx/8, menuBox->infoRects->HP.sy/8, FALSE);
        if (c != 2)
...
static void DisplayPartyPokemonHP(u16 hp, struct PartyMenuBox *menuBox)
{
    ...
    DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0, &menuBox->infoRects->HP);
}
...
static void DisplayPartyPokemonMaxHPCheck(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    {
        if (c != 0)
            menuBox->infoRects->blitFunc(menuBox->windowId, (menuBox->infoRects->maxHP.px/8) + 1, (menuBox->infoRects->maxHP.py/8) + 1, menuBox->infoRects->maxHP.sx/8, menuBox->infoRects->maxHP.sy/8, FALSE);
        if (c != 2)
...
static void DisplayPartyPokemonMaxHP(u16 maxhp, struct PartyMenuBox *menuBox)
{
    ...
    StringAppend(gStringVar1, gStringVar2);
    DisplayPartyPokemonBarDetail(menuBox->windowId, gStringVar1, 0, &menuBox->infoRects->maxHP);
}
...
static void DisplayPartyPokemonHPBar(u16 hp, u16 maxhp, struct PartyMenuBox *menuBox)
{
    ...
    hpFraction = GetScaledHPFraction(hp, maxhp, menuBox->infoRects->hpBar.sx);
    FillWindowPixelRect(menuBox->windowId, sHPBarPalOffsets[1], menuBox->infoRects->hpBar.px, menuBox->infoRects->hpBar.py, hpFraction, 1);
    FillWindowPixelRect(menuBox->windowId, sHPBarPalOffsets[0], menuBox->infoRects->hpBar.px, menuBox->infoRects->hpBar.py + 1, hpFraction, 2);
    if (hpFraction != menuBox->infoRects->hpBar.sx)
    {
        // This appears to be an alternating fill
        FillWindowPixelRect(menuBox->windowId, 0x0D, menuBox->infoRects->hpBar.px + hpFraction, menuBox->infoRects->hpBar.py, menuBox->infoRects->hpBar.sx - hpFraction, 1);
        FillWindowPixelRect(menuBox->windowId, 0x02, menuBox->infoRects->hpBar.px + hpFraction, menuBox->infoRects->hpBar.py + 1, menuBox->infoRects->hpBar.sx - hpFraction, 2);
    }
...
static void DisplayPartyPokemonDescriptionText(u8 stringID, struct PartyMenuBox *menuBox, u8 c)
{
    if (c)
    {
        int width = ((menuBox->infoRects->desc.px % 8) + menuBox->infoRects->desc.sx + 7) / 8;
        int height = ((menuBox->infoRects->desc.py % 8) + menuBox->infoRects->desc.sy + 7) / 8;
        menuBox->infoRects->blitFunc(menuBox->windowId, menuBox->infoRects->desc.px/8, menuBox->infoRects->desc.py/8, width, height, TRUE);
    }
    if (c != 2)
        AddTextPrinterParameterized3(menuBox->windowId, FONT_NORMAL, menuBox->infoRects->desc.px, menuBox->infoRects->desc.py, sFontColorTable[0], 0, sDescriptionStringTable[stringID]);
}
...
static void SetPartyMonSelectionActions(u8 slotId, u8 action)
{
    u32 i;

    if (action == ACTIONS_NONE)
    {
        SetPartyMonFieldSelectionActions(slotId);
        return;
    }
    else
    {
...
static void SetPartyMonFieldSelectionActions(u8 slotId)
{
    u8 i, j;
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            if (GetMonData(&gPlayerParty.party[slotId], i + MON_DATA_MOVE1) == sFieldMoves[j])
            {
    ...
    if (!InBattlePike())
    {
        if (GetMonData(&gPlayerParty.party[1], MON_DATA_SPECIES) != SPECIES_NONE)
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, MENU_SWITCH);
        if (ItemIsMail(GetMonData(&gPlayerParty.party[slotId], MON_DATA_HELD_ITEM)))
...
static void CB2_ShowPokemonSummaryScreen(void)
{
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_IN_BATTLE)
    {
        ...
        ShowPokemonSummaryScreen(SUMMARY_MODE_LOCK_MOVES, gPlayerParty.party, gPartyMenu.slotId, gPlayerParty.count - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
    }
    else
    {
        ShowPokemonSummaryScreen(SUMMARY_MODE_NORMAL, gPlayerParty.party, gPartyMenu.slotId, gPlayerParty.count - 1, CB2_ReturnToPartyMenuFromSummaryScreen);
    }
}
...
static void SwitchSelectedMons(u8 taskId)
{
    ...
    else
    {
        ...
        if (tSlot2Width == 10)
            ...
            tSlot2SlideDir = 1;
        sSlot1TilemapBuffer = Alloc(tSlot1Width * tSlot1Height * sizeof(u16));
        sSlot2TilemapBuffer = Alloc(tSlot2Width * tSlot2Height * sizeof(u16));
...
static void CB2_GiveHoldItem(void)
{
    ...
    {
        sPartyMenuItemId = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_HELD_ITEM);
        ...
        else if (ItemIsMail(gSpecialVar_ItemId))
        {
            ...
            GiveItemToMon(&gPlayerParty.party[gPartyMenu.slotId], gSpecialVar_ItemId);
...
static void Task_GiveHoldItem(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        DisplayGaveHeldItemMessage(&gPlayerParty.party[gPartyMenu.slotId], item, FALSE);
        GiveItemToMon(&gPlayerParty.party[gPartyMenu.slotId], item);
...
static void Task_SwitchHoldItemsPrompt(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        DisplayAlreadyHoldingItemSwitchMessage(&gPlayerParty.party[gPartyMenu.slotId], sPartyMenuItemId, TRUE);
...
static void CB2_WriteMailToGiveMon(void)
{
    u8 mail = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_MAIL);

    DoEasyChatScreen(
        EASY_CHAT_TYPE_MAIL,
        gSaveBlock1.mail[mail].words,
...
static void Task_DisplayGaveMailFromPartyMessage(u8 taskId)
{
    ...
    {
        if (sPartyMenuItemId == ITEM_NONE)
            DisplayGaveHeldItemMessage(&gPlayerParty.party[gPartyMenu.slotId], gSpecialVar_ItemId, FALSE);
        else
...
static void CB2_ReadHeldMail(void)
{
    ReadMail(&gSaveBlock1.mail[GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_MAIL)], CB2_ReturnToPartyMenuFromReadingMail, TRUE);
}
...
static void Task_HandleSendMailToPCYesNoInput(u8 taskId)
{
    ...
    case 0: // Yes, send to PC
        if (TakeMailFromMonAndSave(&gPlayerParty.party[gPartyMenu.slotId]) != MAIL_NONE)
        {
...
static void Task_HandleLoseMailMessageYesNoInput(u8 taskId)
{
    ...
    case 0: // Yes, lose mail message
        item = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_HELD_ITEM);
        if (AddBagItem(item, 1) == TRUE)
        {
            TakeMailFromMon(&gPlayerParty.party[gPartyMenu.slotId]);
...
{
    u16 species2 = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_SPECIES_OR_EGG);
    u16 species = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_SPECIES);
    u8 isModernFatefulEncounter = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_MODERN_FATEFUL_ENCOUNTER);
...
static void CursorCb_Trade1(u8 taskId)
{
    ...
    u16 species = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_SPECIES);
    u8 isModernFatefulEncounter = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_MODERN_FATEFUL_ENCOUNTER);
    int stringId = GetUnionRoomTradeMessageId(*(struct RfuGameCompatibilityData *)GetHostRfuGameData(), gRfuPartnerCompatibilityData, species2, gUnionRoomOfferedSpecies, gUnionRoomRequestedMonType, species, isModernFatefulEncounter);
...
/* static void CursorCb_Trade2(u8 taskId)
{
    ...
    default: // CAN_TRADE_MON
        PlaySE(SE_SELECT);
        GetMonNickname(&gPlayerParty.party[gPartyMenu.slotId], gStringVar1);
...
static u16 GetFieldMoveMonSpecies(void)
{
    return GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_SPECIES);
}
...
void DrawHeldItemIconsForTrade(u8 *partyCounts, u8 *partySpriteIds, u8 whichParty)
{
    ...
    case TRADE_PARTNER:
        ...
        {
            item = GetMonData(&gEnemyParty.party[i], MON_DATA_HELD_ITEM);
            if (item != ITEM_NONE)
...
static bool8 IsHPRecoveryItem(u16 item)
{
    ...
    if (item == ITEM_ENIGMA_BERRY)
        effect = gSaveBlock1.enigmaBerry.itemEffect;
    else
...
{
    if (gMain.inBattle)
        return ExecuteTableBasedItemEffect(&gPlayerParty.party[partyMonIndex], item, GetPartyIdFromBattleSlot(partyMonIndex), monMoveIndex);
    else
        return ExecuteTableBasedItemEffect(&gPlayerParty.party[partyMonIndex], item, partyMonIndex, monMoveIndex);
}
...
static void ShowMoveSelectWindow(u8 slot)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        move = GetMonData(&gPlayerParty.party[slot], MON_DATA_MOVE1 + i);
...
static void TryUsePPItem(u8 taskId)
{
    ...
    if (ExecuteTableBasedItemEffect_(gPartyMenu.slotId, item, *moveSlot))
    {
        gPartyMenuUseExitCallback = FALSE;
        PlaySE(SE_SELECT);
        DisplayPartyMenuMessage(gText_WontHaveEffect, TRUE);
    ...
    {
        gPartyMenuUseExitCallback = TRUE;
        mon = &gPlayerParty.party[gPartyMenu.slotId];
        PlaySE(SE_USE_ITEM);
        RemoveBagItem(item, 1);
...
static void CB2_ShowSummaryScreenToForgetMove(void)
{
    ShowSelectMovePokemonSummaryScreen(gPartyMenu.slotId, gPlayerParty.count - 1, CB2_ReturnToPartyMenuWhileLearningMove, gPartyMenu.data1);
}
...
static void Task_TryLearnNewMoves(u8 taskId)
{
    ...
    if (WaitFanfare(FALSE) && (JOY_NEW(A_BUTTON | B_BUTTON)))
    {
        ...
        learnMove = MonTryLearningNewMove(&gPlayerParty.party[gPartyMenu.slotId], TRUE);
...
{
    u16 result = MonTryLearningNewMove(&gPlayerParty.party[gPartyMenu.slotId], FALSE);
...
static void Task_UpdateHeldItemSpriteAndClosePartyMenu(u8 taskId)
{
    ...
    if (IsPartyMenuTextPrinterActive() != TRUE)
    {
        UpdatePartyMonHeldItemSprite(&gPlayerParty.party[slot], &sPartyMenuBoxes[slot]);
        Task_ClosePartyMenu(taskId);
    }
}
...
static void CB2_WriteMailToGiveMonFromBag(void)
{
    ...
    GiveItemToMon(&gPlayerParty.party[gPartyMenu.slotId], gPartyMenu.bagItem);
    mail = GetMonData(&gPlayerParty.party[gPartyMenu.slotId], MON_DATA_MAIL);
    DoEasyChatScreen(
        EASY_CHAT_TYPE_MAIL,
        gSaveBlock1.mail[mail].words,
...
static void Task_DisplayGaveMailFromBagMessage(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        if (sPartyMenuItemId != ITEM_NONE)
            ...
            DisplayGaveHeldItemMessage(&gPlayerParty.party[gPartyMenu.slotId], gPartyMenu.bagItem, FALSE);
        gTasks[taskId].func = Task_UpdateHeldItemSpriteAndClosePartyMenu;
    }
}
...
static void TryGiveMailToSelectedMon(u8 taskId)
{
    ...
    gPartyMenuUseExitCallback = FALSE;
    mail = &gSaveBlock1.mail[gPlayerPCItemPageInfo.itemsAbove + PARTY_SIZE + gPlayerPCItemPageInfo.cursorPos];
    if (GetMonData(mon, MON_DATA_HELD_ITEM) != ITEM_NONE)
    {
...
{
    if (GetBattleEntryEligibility(&gPlayerParty.party[slot]) == FALSE)
        return 2;
...
static bool8 GetBattleEntryEligibility(struct Pokemon *mon)
{
    ...
    if (GetMonData(mon, MON_DATA_IS_EGG)
        ...
        || (gSaveBlock1.location.mapGroup == MAP_GROUP(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY)
            && gSaveBlock1.location.mapNum == MAP_NUM(BATTLE_FRONTIER_BATTLE_PYRAMID_LOBBY)
            && GetMonData(mon, MON_DATA_HELD_ITEM) != ITEM_NONE))
    {
        return FALSE;
    }

    u16 var = VarGet(VAR_FRONTIER_FACILITY);

    if (var == FACILITY_UNION_ROOM)
        return TRUE;
    if (var == FACILITY_MULTI_OR_EREADER)
    {
        if (GetMonData(mon, MON_DATA_HP) == 0)
            return FALSE;
    }
...
static u8 GetPartyLayoutFromBattleType(void)
{
    if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) == FALSE)
...
{
    if (GetMonData(&gPlayerParty.party[1], MON_DATA_SPECIES) != SPECIES_NONE && GetMonData(mon, MON_DATA_IS_EGG) == FALSE)
    {
        if (gPartyMenu.action == PARTY_ACTION_SEND_OUT)
...
static bool8 TrySwitchInPokemon(void)
{
    ...
    if (GetMonData(&gPlayerParty.party[slot], MON_DATA_HP) == 0)
    {
        GetMonNickname(&gPlayerParty.party[slot], gStringVar1);
    ...
    if (gPartyMenu.action == PARTY_ACTION_CANT_SWITCH)
    {
        ...
        GetMonNickname(&gPlayerParty.party[GetPartyIdFromBattlePartyId(gBattlerPartyIndexes[currBattler])], gStringVar1);
    ...
    SwapPartyPokemon(&gPlayerParty.party[newSlot], &gPlayerParty.party[slot]);
    return TRUE;
}
...
static void BufferBattlePartyOrder(u8 *partyBattleOrder, u8 flankId)
{
    ...
    else if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) == FALSE)
    {
        j = 1;
    ...
    for (i = 0; i < ARRAY_COUNT(gBattlePartyCurrentOrder); i++)
        partyBattleOrder[i] = (partyIds[0 + (i << 1)] << 4) | partyIds[1 + (i << 1)];
}
...
static void BufferBattlePartyOrderBySide(u8 *partyBattleOrder, u8 flankId, u8 battlerId)
{
    ...
    for (i = 0; i < 3; i++)
        partyBattleOrder[i] = (partyIndexes[0 + (i << 1)] << 4) | partyIndexes[1 + (i << 1)];
}
...
{
    u8 partyId1 = GetPartyIdFromBattleSlot(slot);
    u8 partyId2 = GetPartyIdFromBattleSlot(slot2);
    SetPartyIdAtBattleSlot(slot, partyId2);
    SetPartyIdAtBattleSlot(slot2, partyId1);
}
...
u8 GetPartyIdFromBattlePartyId(u8 battlePartyId)
{
    ...
    for (j = i = 0; i < ARRAY_COUNT(gBattlePartyCurrentOrder); i++)
    {
...
static void UpdatePartyToBattleOrder(void)
{
    struct Pokemon *partyBuffer = Alloc(sizeof(gPlayerParty.party));
    u8 i;

    memcpy(partyBuffer, gPlayerParty.party, sizeof(gPlayerParty.party));
    for (i = 0; i < PARTY_SIZE; i++)
        memcpy(&gPlayerParty.party[GetPartyIdFromBattlePartyId(i)], &partyBuffer[i], sizeof(struct Pokemon));
    Free(partyBuffer);
}
...
static void UpdatePartyToFieldOrder(void)
{
    ...
    memcpy(partyBuffer, gPlayerParty.party, sizeof(gPlayerParty.party));
    for (i = 0; i < PARTY_SIZE; i++)
        memcpy(&gPlayerParty.party[GetPartyIdFromBattleSlot(i)], &partyBuffer[i], sizeof(struct Pokemon));
    Free(partyBuffer);
}
...
static void UNUSED SwitchAliveMonIntoLeadSlot(void)
{
    ...
    {
        mon = &gPlayerParty.party[GetPartyIdFromBattleSlot(i)];
        if (GetMonData(mon, MON_DATA_SPECIES) != SPECIES_NONE && GetMonData(mon, MON_DATA_HP) != 0)
        {
            ...
            SwapPartyPokemon(&gPlayerParty.party[partyId], mon);
            break;
        }
    }
}
...
static void CB2_ChooseMonForMoveRelearner(void)
{
    ...
    if (gSpecialVar_0x8004 >= PARTY_SIZE)
        ...
        gSpecialVar_0x8005 = GetNumberOfRelearnableMoves(&gPlayerParty.party[gSpecialVar_0x8004]);
    gFieldCallback2 = CB2_FadeFromPartyMenu;
    SetMainCallback2(CB2_ReturnToField);
}
...
void DoBattlePyramidMonsHaveHeldItem(void)
{
    ...
    {
        if (GetMonData(&gPlayerParty.party[i], MON_DATA_HELD_ITEM) != ITEM_NONE)
        {
            gSpecialVar_Result = TRUE;
            break;
        }
...
{
    ShowPokemonSummaryScreen(SUMMARY_MODE_SELECT_MOVE, gPlayerParty.party, gSpecialVar_0x8004, gPlayerParty.count - 1, CB2_ReturnToField);
...
void GetNumMovesSelectedMonHas(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (GetMonData(&gPlayerParty.party[gSpecialVar_0x8004], MON_DATA_MOVE1 + i) != MOVE_NONE)
            gSpecialVar_Result++;
    }
}
...
{
    struct Pokemon *mon = &gPlayerParty.party[gSpecialVar_0x8004];
...
void MoveDeleterForgetMove(void)
{
    ...
    SetMonMoveSlot(&gPlayerParty.party[gSpecialVar_0x8004], MOVE_NONE, gSpecialVar_0x8005);
    RemoveMonPPBonus(&gPlayerParty.party[gSpecialVar_0x8004], gSpecialVar_0x8005);
    for (i = gSpecialVar_0x8005; i < MAX_MON_MOVES - 1; i++)
        ShiftMoveSlot(&gPlayerParty.party[gSpecialVar_0x8004], i, i + 1);
}
...
{
    if (GetMonData(&gPlayerParty.party[gSpecialVar_0x8004], MON_DATA_IS_EGG))
        gSpecialVar_Result = TRUE;
    else
        gSpecialVar_Result = FALSE;
}
...
void IsLastMonThatKnowsSurf(void)
{
    ...
    gSpecialVar_Result = FALSE;
    move = GetMonData(&gPlayerParty.party[gSpecialVar_0x8004], MON_DATA_MOVE1 + gSpecialVar_0x8005);
    if (move == MOVE_SURF)
    {
        for (i = 0; i < gPlayerParty.count; i++)
        {
            if (i != gSpecialVar_0x8004)
            {
                ...
                {
                    if (GetMonData(&gPlayerParty.party[i], MON_DATA_MOVE1 + j) == MOVE_SURF)
                        return;
                }
            }
        }
