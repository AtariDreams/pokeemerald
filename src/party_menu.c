...
static u8 CanMonLearnTMTutor(struct Pokemon *mon, u16 item, u8 tutor)
{
    ...
    {
        if (!CanLearnTutorMove(GetMonData(mon, MON_DATA_SPECIES, NULL), tutor))
            return CANNOT_LEARN_MOVE;
        else
...
static void DisplayPartyPokemonHPCheck(struct Pokemon *mon, struct PartyMenuBox *menuBox, u8 c)
{
    if (GetMonData(mon, MON_DATA_SPECIES, NULL) != SPECIES_NONE)
    {
        ...
        if (c != 2)
            DisplayPartyPokemonHP(GetMonData(mon, MON_DATA_HP, NULL), menuBox);
    }
}
...
static void CB2_ReadHeldMail(void)
{
    ReadMail(&gSaveBlock1.mail[GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_MAIL, NULL)], CB2_ReturnToPartyMenuFromReadingMail, 1);
}
...
{
    u16 species2 = GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_SPECIES2, NULL);
    u16 species = GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_SPECIES, NULL);
    u8 isEventLegal = GetMonData(&gPlayerParty[gPartyMenu.slotId], MON_DATA_EVENT_LEGAL, NULL);
...
static void CreatePartyMonIconSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox, u32 slot)
{
    ...
    species2 = GetMonData(mon, MON_DATA_SPECIES2, NULL);
    CreatePartyMonIconSpriteParameterized(species2, GetMonData(mon, MON_DATA_PERSONALITY, NULL), menuBox, 1, handleDeoxys);
