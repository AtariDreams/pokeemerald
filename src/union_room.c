...
static void PrintPlayerNameAndIdOnWindow(u8 windowId)
{
    ...
    PrintUnionRoomText(windowId, FONT_NORMAL, gSaveBlock2.playerName, 0, 1, UR_COLOR_DEFAULT);
...
static void Task_TryBecomeLinkLeader(u8 taskId)
{
    ...
    {
    case LL_STATE_INIT:
        if (gSpecialVar_0x8004 == LINK_GROUP_BATTLE_TOWER && gSaveBlock2.frontier.lvlMode == FRONTIER_LVL_OPEN)
            gSpecialVar_0x8004++;
...
static u32 IsTryingToTradeAcrossVersionTooSoon(struct WirelessLink_Group *data, s32 id)
{
    ...
    {
        if (!(gSaveBlock2.specialSaveWarpFlags & CHAMPION_SAVEWARP))
...
static void Task_StartUnionRoomTrade(u8 taskId)
{
    ...
    case 1:
        ...
        break;
    case 2:
        memcpy(gBlockSendBuffer, gSaveBlock1.mail, sizeof(struct Mail) * PARTY_SIZE + 4);
...
static void WarpForWirelessMinigame(u16 linkService, u16 x, u16 y)
{
    ...
    SetWarpDestination(gSaveBlock1.location.mapGroup, gSaveBlock1.location.mapNum, WARP_ID_NONE, x, y);
    SetDynamicWarpWithCoords(0, gSaveBlock1.location.mapGroup, gSaveBlock1.location.mapNum, WARP_ID_NONE, x, y);
    WarpIntoMap();
}
...
static void TradeBoardListMenuItemPrintFunc(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId == LIST_HEADER && y == sTradeBoardListMenuTemplate.upText_Y)
    {
        ...
        if (gameData->tradeSpecies != SPECIES_NONE)
            TradeBoardPrintItemInfo(windowId, y, gameData, gSaveBlock2.playerName, UR_COLOR_TRADE_BOARD_SELF);
    }
    else
    {
        j = 0;
...
static void ViewURoomPartnerTrainerCard(u8 *unused, struct WirelessLink_URoom *data, bool8 isParent)
{
    ...
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(1, trainerCard->basic.playerName);

    StringCopy(data->trainerCardColorStrBuffer, sCardColorTexts[trainerCard->basic.stars]);
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(2, data->trainerCardColorStrBuffer);

    ConvertIntToDecimalStringN(data->trainerCardStrBuffer[2], trainerCard->basic.caughtMonsCount, STR_CONV_MODE_LEFT_ALIGN, 3);
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(3, data->trainerCardStrBuffer[2]);

    ConvertIntToDecimalStringN(data->trainerCardStrBuffer[3], trainerCard->basic.playTimeHours, STR_CONV_MODE_LEFT_ALIGN, 3);
    ConvertIntToDecimalStringN(data->trainerCardStrBuffer[4], trainerCard->basic.playTimeMinutes, STR_CONV_MODE_LEADING_ZEROS, 2);
    ...
    ConvertIntToDecimalStringN(data->trainerCardStrBuffer[2], trainerCard->basic.pokemonTrades, STR_CONV_MODE_LEFT_ALIGN, 5);
    DynamicPlaceholderTextUtil_SetPlaceholderPtr(3, data->trainerCardStrBuffer[2]);

    for (i = 0; i < TRAINER_CARD_PROFILE_LENGTH; i++)
    {
        CopyEasyChatWord(data->trainerCardStrBuffer[i + 3], trainerCard->basic.easyChatProfile[i]);
    ...
    {
        DynamicPlaceholderTextUtil_ExpandPlaceholders(data->trainerCardMsgStrBuffer, sGladToMeetYouTexts[trainerCard->basic.gender]);
        StringAppend(gStringVar4, data->trainerCardMsgStrBuffer);
    }
}
