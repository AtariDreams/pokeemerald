...
{
    return gSaveBlock1.lilycoveLady.id;
}
...
{
    u16 id = ((gSaveBlock2.playerTrainerId[1] << 8) | gSaveBlock2.playerTrainerId[0]);
    id %= 6;
    id >>= 1;
    switch (id)
    {
...
static void InitLilycoveFavorLady(void)
{
    sFavorLadyPtr = &gSaveBlock1.lilycoveLady.favor;
...
static bool8 DoesFavorLadyLikeItem(u16 itemId)
{
    ...
    SetFavorLadyPlayerName(gSaveBlock2.playerName, sFavorLadyPtr->playerName);
    sFavorLadyPtr->language = gGameLanguage;
    likedItem = FALSE;
...
static void InitLilycoveQuizLady(void)
{
    ...
    sQuizLadyPtr = &gSaveBlock1.lilycoveLady.quiz;
...
u8 GetQuizAuthor(void)
{
    s32 i, j;
    u8 authorNameId;
    struct LilycoveLadyQuiz *quiz = &gSaveBlock1.lilycoveLady.quiz;
...
static u8 BufferQuizAuthorName(void)
{
    ...
    else
    {
        ...
        if (nameLen == GetPlayerNameLength(gSaveBlock2.playerName))
        {
            u8 *name = sQuizLadyPtr->playerName;
            for (i = 0; i < nameLen; i++)
            {
                name = sQuizLadyPtr->playerName;
                if (name[i] != gSaveBlock2.playerName[i])
                {
...
static bool8 IsQuizTrainerIdNotPlayer(void)
{
    ...
    for (i = 0; i < TRAINER_ID_LENGTH; i++)
    {
        if (sQuizLadyPtr->playerTrainerId[i] != gSaveBlock2.playerTrainerId[i])
        {
            notPlayer = TRUE;
            break;
        }
...
void QuizLadyRecordCustomQuizData(void)
{
    ...
    for (i = 0; i < TRAINER_ID_LENGTH; i++)
        sQuizLadyPtr->playerTrainerId[i] = gSaveBlock2.playerTrainerId[i];
    StringCopy_PlayerName(sQuizLadyPtr->playerName, gSaveBlock2.playerName);
    sQuizLadyPtr->language = gGameLanguage;
}
...
static void InitLilycoveContestLady(void)
{
    sContestLadyPtr = &gSaveBlock1.lilycoveLady.contest;
