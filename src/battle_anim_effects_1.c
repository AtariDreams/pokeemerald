...
#if !MODERN
static void AnimMilkBottle_Step2(struct Sprite *, s16, s16);
#else
static void AnimMilkBottle_Step2(struct Sprite *);
#endif
...
static void AnimHyperBeamOrb(struct Sprite* sprite)
{
    ...
    // intended to differ between negative numbers as well as positive but that isn't how it worked out
    #if !MODERN
    speed = Random2() % 32;
    sprite->data[0] = speed + 64;
    #else
    sprite->data[0] = (Random2() & 31) + 64;
    #endif
    sprite->data[1] = sprite->x;
    sprite->data[2] = GetBattlerSpriteCoord(gBattleAnimTarget, BATTLER_COORD_X_2);
    sprite->data[3] = sprite->y;
...
static void AnimSporeParticle_Step(struct Sprite* sprite)
{
    ...
    if (sprite->data[1] > 63 &&  sprite->data[1] < 192)
    {
...
static void AnimPetalDanceSmallFlower_Step(struct Sprite* sprite)
{
    if (!AnimTranslateLinear(sprite))
    {
        ...
        if ((sprite->data[5] < 64 && sprite->data[5] + 5 >= 64) || (sprite->data[5] < 192 && sprite->data[5] + 5 >= 192))
...
static void AnimNeedleArmSpike(struct Sprite *sprite)
{
    ...
    else
    {
        sprite->x = a;
        sprite->y = b;
        sprite->data[5] = a + gBattleAnimArgs[2];
        sprite->data[6] = b + gBattleAnimArgs[3];
    }

    sprite->data[1] = sprite->x << 4;
    sprite->data[2] = sprite->y << 4;
    sprite->data[3] = ((sprite->data[5] - sprite->x) << 4) / gBattleAnimArgs[4];
    sprite->data[4] = ((sprite->data[6] - sprite->y) << 4) / gBattleAnimArgs[4];
    c = ArcTan2Neg(sprite->data[5] - sprite->x, sprite->data[6] - sprite->y);
    if (IsContest())
        c -= 0x8000;
...
#if !MODERN
static void AnimMilkBottle_Step2(struct Sprite* sprite, s16 unk1, s16 unk2)
#else
static void AnimMilkBottle_Step2(struct Sprite* sprite)
#endif
{
    if (sprite->data[3] < 12)
        sprite->data[4] += 2;
...
static void AnimLockOnMoveTarget(struct Sprite* sprite)
{
    ...
    if (sprite->oam.affineParam == 1)
    {
        sprite->x -= 0x18;
        sprite->y -= 0x18;
    }
    else if (sprite->oam.affineParam == 2)
    {
        sprite->x -= 0x18;
        sprite->y += 0x18;
        sprite->oam.matrixNum = ST_OAM_VFLIP;
    }
    else if (sprite->oam.affineParam == 3)
    {
        sprite->x += 0x18;
        sprite->y -= 0x18;
        sprite->oam.matrixNum = ST_OAM_HFLIP;
...
{
    u32 a = GetBattleBgPalettesMask(TRUE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE) & 0xFFFF;
    u32 b;
...
static void AnimFollowMeFinger_Step2(struct Sprite* sprite)
{
    ...
    sprite->x2 = gSineTable[sprite->data[1]] >> 3;
...
void AnimTask_AirCutterProjectile(u8 taskId)
{
    ...
    else if ((gBattlerPositions[gBattleAnimTarget] & BIT_SIDE) == B_SIDE_PLAYER)
    {
    ...
    if (gBattleAnimArgs[4] & 0x80)
    {
        ...
        if (gBattleAnimArgs[4] >= 64)
        {
            gTasks[taskId].data[2] = GetBattlerSpriteSubpriority(gBattleAnimTarget) + (gBattleAnimArgs[4] - 64);
        }
        else
        {
            gTasks[taskId].data[2] = GetBattlerSpriteSubpriority(gBattleAnimTarget) - gBattleAnimArgs[4];
        }
    }
    else
    {
...
static void AnimBulletSeed_Step1(struct Sprite *sprite)
{
    ...
    sprite->data[6] = -12 - (Random2() % 8);
    sprite->data[7] = 0xA0 + (Random2() % 0xA0);
...
{
    s16 startXOffset, startYOffset;
    s16 endXOffset, endYOffset;
    #if !MODERN
    startXOffset = 32;
    startYOffset = -32;
    endXOffset = 16;
...
{
    struct Task* task = &gTasks[taskId];
    PrepareAffineAnimInTaskData(task, GetAnimBattlerSpriteId(ANIM_ATTACKER), gGrowAndShrinkAffineAnimCmds);
...
{
    #if !MODERN
    u16 index = IndexOfSpritePaletteTag(ANIM_TAG_THIN_RING);
    if (index != 0xFF)
    {
        index = 256 + index * 16 + 1;
        BlendPalette(index, 15, gBattleAnimArgs[5], gBattleAnimArgs[4]);
    }
    #else
    u8 index = IndexOfSpritePaletteTag(ANIM_TAG_THIN_RING);
    if (index != 0xFF)
    {
        BlendPalette(256 + index * 16 + 1, 15, gBattleAnimArgs[5], gBattleAnimArgs[4]);
    }
    #endif
...
void AnimTask_HeartsBackground(u8 taskId)
{
    ...
    // TODO: apparently this was a debug
    #ifndef CON_DEBUG
...
static void AnimJaggedMusicNote(struct Sprite *sprite)
{
    ...
    // TODO: Does this change observable behavior?
    #if !MODERN
    sprite->data[3] = (gBattleAnimArgs[1] << 3) / 8; // why??

    sprite->data[4] = (gBattleAnimArgs[2] << 3) / 8; // why???
    #else
