...
void AnimTask_CreateSmallSolarBeamOrbs(u8 taskId)
{
    if (gTasks[taskId].data[0]-- == 0)
    {
        ...
        gBattleAnimArgs[3] = 0;
        CreateSpriteAndAnimatePointer(&gSolarBeamSmallOrbSpriteTemplate, 0, 0, GetBattlerSpriteSubpriority(gBattleAnimTarget) + 1);
    }
...
static void AnimPetalDanceSmallFlower_Step(struct Sprite *sprite)
{
    if (!AnimTranslateLinear(sprite))
    {
        ...
        if ((sprite->data[5] < 64 && sprite->data[5] >= 59) || (sprite->data[5] < 192 && sprite->data[5] >= 187))
...
static void AnimTranslateLinearSingleSineWave_Step(struct Sprite *sprite)
{
    ...
    {
        sprite->invisible ^= 1; // It already rotated once, so turn it off
        // Todo: just set to false?
        sprite->oam.affineParam++;
...
static void AnimIngrainOrb(struct Sprite *sprite)
{
    ...
    sprite->data[0]++;
    sprite->x2 = sprite->data[0] * sprite->data[1];
...
bool8 moveAlongLinearPath(struct Sprite *sprite)
{
    ...
    u16 yStartPos = (u8)(sprite->data[5] & 0xFF);
...
static void AnimPresentHealParticle(struct Sprite *sprite)
{
    ...
    sprite->data[0]++;
    sprite->y2 = sprite->data[0] * sprite->data[1];
    if (sprite->animEnded)
        DestroyAnimSprite(sprite);
}
...
static void AnimTrickBag(struct Sprite *sprite)
{
    ...
    if (!sprite->data[0])
    {
        ...
        {
            // Todo: should it be a bitwise operation?
...
void AnimTask_LeafBlade(u8 taskId)
{
    ...
    task->data[8] = task->data[7] + (task->data[6] - task->data[9]);
...
static void AnimTask_LeafBlade_Step2(struct Task *task, u8 taskId)
{
    if (++task->data[14] > 0)
    {
        ...
        sprite = CreateSpriteReturnPointer(&gLeafBladeSpriteTemplate, spriteX, spriteY, task->data[4]);
        if (sprite != NULL)
        {
            sprite->data[6] = taskId;
            sprite->data[7] = 12;
            sprite->data[12]++;
            sprite->data[0] = task->data[13] & 1;
            sprite->data[13]++;
            StartSpriteAnim(sprite, task->data[3]);
            sprite->subpriority = task->data[4];
            sprite->callback = AnimTask_LeafBlade_Step2_Callback;
        }
    }
}
...
static void AnimFlyingParticle_Step(struct Sprite *sprite)
{
    ...
    sprite->x2 = a * sprite->data[2];
    sprite->data[0] = (a * sprite->data[3]) & 0xFF;
    if (!sprite->data[4])
    {
...
static void AnimNeedleArmSpike(struct Sprite *sprite)
{
    ...
    else
    {
        ...
        {
            sprite->x = a + gBattleAnimArgs[2];
            sprite->y = b + gBattleAnimArgs[3];
            sprite->data[5] = a;
            sprite->data[6] = b;
        }
        else
...
static void AnimWavyMusicNotes_Step(struct Sprite *sprite)
{
    ...
    sprite->sMoveTimer++;
    trigIdx = (sprite->sMoveTimer * 5) & 0xFF;
...
void AnimSlowFlyingMusicNotes(struct Sprite *sprite)
{
    ...
    sprite->data[0] = 40;
    sprite->data[1] = sprite->x;
    sprite->data[2] = sprite->data[1] + xDiff;
    sprite->data[3] = sprite->y;
