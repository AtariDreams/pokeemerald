...
// TODO: Check constness after main code changes
static void InitBackupMapLayoutData(u16 *map, u16 width, u16 height)
{
    u16 *dest;
    m32 y;
...
static void InitBackupMapLayoutConnections(struct MapHeader *mapHeader)
{
    ...
    if (mapHeader->connections)
    {
        ...
        {
            const struct MapHeader *cMap = GetMapHeaderFromConnection(connection);
...
static void FillSouthConnection(struct MapHeader const *mapHeader, struct MapHeader const *connectedMapHeader, s32 offset)
{
    ...
    if (connectedMapHeader)
    {
        ...
        // the 0 can be replaced with cHeight, which is inlined in calculating y, but GF removed it for some reason and replaced it with 0?
        FillConnection(
            x, y,
            connectedMapHeader,
            x2, /*y2*/ 0,
...
static void FillNorthConnection(struct MapHeader const *mapHeader, struct MapHeader const *connectedMapHeader, s32 offset)
{
    ...
    if (connectedMapHeader)
    {
        ...
        // The OG has nothing commented out, but the version of this in the function above does have an assignment commented out. Probably for consistency in the loading mechanic
        FillConnection(
            x, /*y*/ 0,
            connectedMapHeader,
            x2, y2,
...
bool8 MapGridGetCollisionAt(int x, int y)
{
    u16 block = GetMapGridBlockAt(x, y);
...
static void MoveMapViewToBackup(u8 direction)
{
    ...
    for (y = 0; y < y2; y++)
    {
        for (x = 0; x < x2; x++)
        {
            sBackupMapData[x + x0 + width * (y + y0)] = mapView[r9 + x + MAP_OFFSET_W * (y + r8)];
        }
    }
    ClearSavedMapView();
}

// This should be returning u8, but of course -1 being signed and all means it won't match
...
static bool32 IsCoordInConnectingMap(int coord, int max)
{
...
static bool32 IsPosInConnectingMap(struct MapConnection *connection, int x, int y)
{
    struct MapHeader const *mapHeader;
    mapHeader = GetMapHeaderFromConnection(connection);
    switch (connection->direction)
