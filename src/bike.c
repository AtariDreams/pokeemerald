...
// used with bikeFrameCounter from mach bike. Local to a function, but doesn't match unless here
...
static void MachBikeTransition_TurnDirection(u8 direction)
{
    ...
    {
        MachBikeTransition_FaceDirection(playerObjEvent->directions.regDir.facingDirection);
    }
}
...
static void MachBikeTransition_TrySpeedUp(u8 direction)
{
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == FALSE)
    {
        ...
        if (gPlayerAvatar.bikeSpeed)
            MachBikeTransition_TrySlowDown(playerObjEvent->directions.regDir.movementDirection);
        else
            MachBikeTransition_FaceDirection(playerObjEvent->directions.regDir.movementDirection);
    }
    else
    {
        ...
        if (collision != COLLISION_NONE && collision < COLLISION_VERTICAL_RAIL)
        {
            ...
            else
            {
                ...
                else if (collision < COLLISION_STOP_SURFING || collision >= COLLISION_WHEELIE_HOP)
...
static void AcroBikeTransition_TurnDirection(u8 direction)
{
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == 0)
        direction = playerObjEvent->directions.regDir.movementDirection;
    PlayerFaceDirection(direction);
}
...
static void AcroBikeTransition_Moving(u8 direction)
{
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == 0)
    {
        AcroBikeTransition_FaceDirection(playerObjEvent->directions.regDir.movementDirection);
        return;
    }
    collision = GetBikeCollision(direction);
    if (collision != COLLISION_NONE && collision < COLLISION_VERTICAL_RAIL)
...
static void AcroBikeTransition_WheelieHoppingMoving(u8 direction)
{
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == 0)
    {
        AcroBikeTransition_WheelieHoppingStanding(playerObjEvent->directions.regDir.movementDirection);
        return;
    }
    collision = GetBikeCollision(direction);
...
static void AcroBikeTransition_WheelieMoving(u8 direction)
{
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == 0)
    {
        PlayerIdleWheelie(playerObjEvent->directions.regDir.movementDirection);
        return;
    }
    collision = GetBikeCollision(direction);
    if (collision != COLLISION_NONE && collision < COLLISION_VERTICAL_RAIL)
...
static void AcroBikeTransition_WheelieRisingMoving(u8 direction)
{
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == 0)
    {
        PlayerStartWheelie(playerObjEvent->directions.regDir.movementDirection);
        return;
    }
    collision = GetBikeCollision(direction);
    if (collision != COLLISION_NONE && collision < COLLISION_VERTICAL_RAIL)
...
static void AcroBikeTransition_WheelieLoweringMoving(u8 direction)
{
    ...
    if (CanBikeFaceDirOnMetatile(direction, playerObjEvent->currentMetatileBehavior) == 0)
    {
        PlayerEndWheelie(playerObjEvent->directions.regDir.movementDirection);
        return;
    }
    collision = GetBikeCollision(direction);
    if (collision != COLLISION_NONE && collision < COLLISION_VERTICAL_RAIL)
...
static void AcroBike_TryHistoryUpdate(u16 newKeys, u16 heldKeys) // newKeys is unused
{
    ...
    // Devs intended it to just be A or B
...
bool8 IsBikingDisallowedByPlayer(void)
{
    ...
    if ((gPlayerAvatar.flags & (PLAYER_AVATAR_FLAG_SURFING | PLAYER_AVATAR_FLAG_UNDERWATER)) != 0)
    {
        return TRUE;
    }
