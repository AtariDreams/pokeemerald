...
u16 ChooseMoveAndTargetInBattlePalace(void)
{
    ...
    i = (gBattleStruct->palaceFlags & (1U << gActiveBattler)) ? 2 : 0;
    minGroupNum = i;
    ...
    if (chosenMoveId == -1)
    {
        if (unusableMovesBits != ALL_MOVES_MASK)
        {
            ...
            for (i = 0; i < MAX_MON_MOVES; i++)
            {
                ...
                if (GetBattlePalaceMoveGroup(moveInfo->moves[i]) == PALACE_MOVE_GROUP_ATTACK && !((1U << i) & unusableMovesBits))
                    numMovesPerGroup += (1 << 0);
                if (GetBattlePalaceMoveGroup(moveInfo->moves[i]) == PALACE_MOVE_GROUP_DEFENSE && !((1U << i) & unusableMovesBits))
                    numMovesPerGroup += (1 << 4);
                if (GetBattlePalaceMoveGroup(moveInfo->moves[i]) == PALACE_MOVE_GROUP_SUPPORT && !((1U << i) & unusableMovesBits))
            ...
            else
            {
                ...
                {
                    i = Random() % MAX_MON_MOVES;
                    if (!((1U << i) & unusableMovesBits) && randSelectGroup == GetBattlePalaceMoveGroup(moveInfo->moves[i]))
                        chosenMoveId = i;
                } while (chosenMoveId == -1);
            }
...
void BattleLoadSubstituteOrMonSpriteGfx(u8 battlerId, bool8 loadMonSprite)
{
    ...
    if (!loadMonSprite)
    {
        ...
        if (IsContest())
            LZ77UnCompVram(gSubstituteDollBackGfx, gMonSpritesGfxPtr->sprites.ptr[position]);
        else if (GetBattlerSide(battlerId) != B_SIDE_PLAYER)
            LZ77UnCompVram(gSubstituteDollFrontGfx, gMonSpritesGfxPtr->sprites.ptr[position]);
        else
            LZ77UnCompVram(gSubstituteDollBackGfx, gMonSpritesGfxPtr->sprites.ptr[position]);
...
void HandleLowHpMusicChange(struct Pokemon *mon, u8 battlerId)
{
    ...
    else
    {
        ...
        if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) && !gBattleSpritesDataPtr->battlerData[BATTLE_PARTNER(battlerId)].lowHpSong)
        {
            m4aSongNumStop(SE_LOW_HEALTH);
            return;
        }
...
void HandleBattleLowHpMusicChange(void)
{
    if (gMain.inBattle)
    {
        ...
        if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE) && GetMonData(&gPlayerParty.party[battler2PartyId], MON_DATA_HP) != 0)
