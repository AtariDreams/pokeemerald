...
bool8 ScriptMenu_Multichoice(u8 left, u8 top, u8 multichoiceId, bool8 ignoreBPress)
{
    ...
    gSpecialVar_Result = ANS_NOTHING;
...
bool8 ScriptMenu_YesNo(u8 left, u8 top)
{
    ...
    taskId = CreateTask(Task_HandleYesNoInput, 80);
    return TRUE;
}
...
{
    if (gSpecialVar_Result == ANS_NOTHING)
        return FALSE;
    else
        return TRUE;
}
...
static void CreateLilycoveSSTidalMultichoice(void)
{
    ...
    // useless call assigned to an unused variable, hence why it is useless
    #if !MODERN
    ...
    {
        // TODO: would 0 be better since we know this will be 0?
    ...
    if (CheckBagHasItem(ITEM_EON_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_SOUTHERN_ISLAND) == TRUE)
    {
        ...
        M_IF (gSpecialVar_0x8004 == 1 && FlagGet(FLAG_SHOWN_EON_TICKET) == FALSE)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_SOUTHERN_ISLAND;
            selectionCount++;
    ...
    if (CheckBagHasItem(ITEM_MYSTIC_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_NAVEL_ROCK) == TRUE)
    {
        ...
        M_IF (gSpecialVar_0x8004 == 1 && FlagGet(FLAG_SHOWN_MYSTIC_TICKET) == FALSE)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_NAVEL_ROCK;
            selectionCount++;
    ...
    if (CheckBagHasItem(ITEM_AURORA_TICKET, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_BIRTH_ISLAND) == TRUE)
    {
        ...
        M_IF (gSpecialVar_0x8004 == 1 && FlagGet(FLAG_SHOWN_AURORA_TICKET) == FALSE)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_BIRTH_ISLAND;
            selectionCount++;
    ...
    if (CheckBagHasItem(ITEM_OLD_SEA_MAP, 1) == TRUE && FlagGet(FLAG_ENABLE_SHIP_FARAWAY_ISLAND) == TRUE)
    {
        ...
        M_IF (gSpecialVar_0x8004 == 1 && FlagGet(FLAG_SHOWN_OLD_SEA_MAP) == FALSE)
        {
            sLilycoveSSTidalSelections[selectionCount] = SSTIDAL_SELECTION_FARAWAY_ISLAND;
            selectionCount++;
    ...
    // pointless check. TODO: check if bug?
    #if !MODERN
...
int DisplayTextAndGetWidth(const u8 *str, int prevWidth)
{
    ...
    return prevWidth > width ? prevWidth : width;
}

int ConvertPixelWidthToTileWidth(int width)
{
    int w = ((width + 9) / 8) + 1;
    return w > MAX_MULTICHOICE_WIDTH ? MAX_MULTICHOICE_WIDTH : w;
}
