...
EWRAM_DATA struct PokeParty gPlayerParty = {0};
EWRAM_DATA struct PokeParty gEnemyParty = {0};
...
u8 GiveMonToPlayer(struct Pokemon *mon)
{
    ...
    CopyMon(&gPlayerParty.party[gPlayerParty.partyCount++], mon, sizeof(*mon));
...
{
    gPlayerParty.partyCount = 0;

    while (gPlayerParty.partyCount < PARTY_SIZE
        && GetMonData(&gPlayerParty.party[gPlayerParty.partyCount], MON_DATA_SPECIES, NULL) != SPECIES_NONE)
    {
        gPlayerParty.partyCount++;
    }

    return gPlayerParty.partyCount;
}

u8 CalculateEnemyPartyCount(void)
{
    gEnemyParty.partyCount = 0;

    while (gEnemyParty.partyCount < PARTY_SIZE
        && GetMonData(&gEnemyParty.party[gEnemyParty.partyCount], MON_DATA_SPECIES, NULL) != SPECIES_NONE)
    {
        gEnemyParty.partyCount++;
    }

    return gEnemyParty.partyCount;
}
...
u8 GetMonsStateToDoubles(void)
{
    ...
    if (gPlayerParty.partyCount == 1)
        return gPlayerParty.partyCount; // PLAYER_HAS_ONE_MON
