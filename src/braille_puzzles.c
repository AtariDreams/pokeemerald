...
bool8 CheckRelicanthWailord(void)
{
    ...
    // First comes Wailord
    if (GetMonData(&gPlayerParty.party[0], MON_DATA_SPECIES_OR_EGG, 0) == SPECIES_WAILORD)
    {
        CalculatePlayerPartyCount();
        // Last comes Relicanth
        if (GetMonData(&gPlayerParty.party[gPlayerParty.count - 1], MON_DATA_SPECIES_OR_EGG, 0) == SPECIES_RELICANTH)
            return TRUE;
    }
    return FALSE;
}
