...
/*
    ...
    ## Saving trends ##
    Each time a potential trendy phrase is submitted, it is saved in gSaveBlock1.dewfordTrends[].
    ...
    gSaveBlock1.dewfordTrends[0]. If the player mixes records with another player, their own
...
// phrase is always saved in gSaveBlock1.dewfordTrends
...
static bool8 IsPhraseInSavedTrends(u16 *phrase)
{
    ...
    for (i = 0; i < SAVED_TRENDS_COUNT; i++)
    {
        if (IsEasyChatPairEqual(phrase, gSaveBlock1.dewfordTrends[i].words))
            return TRUE;
    }
    return FALSE;
}
