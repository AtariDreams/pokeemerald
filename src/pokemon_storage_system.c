...
u8 CountPartyAliveNonEggMonsExcept(u8 slotToIgnore)
{
    ...
    {
        if (i != slotToIgnore
            && GetMonData(&gPlayerParty.party[i], MON_DATA_SPECIES) != SPECIES_NONE
            && !GetMonData(&gPlayerParty.party[i], MON_DATA_IS_EGG)
...
static void Task_OnCloseBoxPressed(u8 taskId)
{
    ...
    case 4:
        ...
        {
            UpdateBoxToSendMons();
            gPlayerParty.partyCount = CalculatePlayerPartyCount();
...
static void SetPartySlotTilemaps(void)
{
    ...
    {
        s32 species = GetMonData(&gPlayerParty.party[i], MON_DATA_SPECIES);
...
{
    if (boxId == TOTAL_BOXES_COUNT)
        ZeroMonData(&gPlayerParty.party[position]);
    else
...
s16 CompactPartySlots(void)
{
    ...
    for (; last < PARTY_SIZE; last++)
        ZeroMonData(&gPlayerParty.party[last]);
