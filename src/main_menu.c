...
static void NewGameBirchSpeech_ShowDialogueWindow(void);
...
static void Task_HandleMainMenuAPressed(u8 taskId)
{
    ...
    {
        // Todo: Why does Task_NewGameBirchSpeech_Init reuse the task?
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_SALAMENCE);
...
static void Task_NewGameBirchSpeech_ChooseGender(u8 taskId)
{
    ...
    switch (gender)
    {
        ...
        case FEMALE:
            ...
            gTasks[taskId].func = Task_NewGameBirchSpeech_WhatsYourName;
            break;
        default: // Handle nothing chosen yet
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        DestroyTask(taskId);
        DoNamingScreen(NAMING_SCREEN_PLAYER, gSaveBlock2.playerName, gSaveBlock2.playerGender, 0, 0, CB2_NewGameBirchSpeech_ReturnFromNamingScreen);
    }
}
...
{
    if (gSprites[gTasks[taskId].tPlayerSpriteId].affineAnimEnded)
...
static void Task_NewGameBirchSpeech_FadePlayerToWhite(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        gSprites[gTasks[taskId].tPlayerSpriteId].callback = SpriteCB_Null;
        SetGpuReg(REG_OFFSET_DISPCNT, DISPCNT_OBJ_ON | DISPCNT_OBJ_1D_MAP);
...
{
    return CreateMonPicSprite_Affine(SPECIES_SALAMENCE, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    id = AddNewGameBirchObject(0x88, 0x3C, 1);
    gSprites[id].callback = SpriteCB_Null;
    gSprites[id].oam.priority = 0;
    gSprites[id].invisible = TRUE;
    gTasks[taskId].tBirchSpriteId = id;
    id = NewGameBirchSpeech_CreateLotadSprite(100, 0x4B);
    gSprites[id].callback = SpriteCB_Null;
    gSprites[id].oam.priority = 0;
    gSprites[id].invisible = TRUE;
    gTasks[taskId].tLotadSpriteId = id;
    id = CreateTrainerSprite(FacilityClassToPicIndex(FACILITY_CLASS_BRENDAN), 120, 60, 0, &gDecompressionBuffer[0]);
    gSprites[id].callback = SpriteCB_Null;
    gSprites[id].invisible = TRUE;
    gSprites[id].oam.priority = 0;
    gTasks[taskId].tBrendanSpriteId = id;
    id = CreateTrainerSprite(FacilityClassToPicIndex(FACILITY_CLASS_MAY), 120, 60, 0, &gDecompressionBuffer[TRAINER_PIC_SIZE]);
    gSprites[id].callback = SpriteCB_Null;
    gSprites[id].invisible = TRUE;
    gSprites[id].oam.priority = 0;
    gTasks[taskId].tMaySpriteId = id;
}
...
{
    if (gSaveBlock2.playerGender == MALE)
        StringCopy(gSaveBlock2.playerName, gText_ExpandedPlaceholder_Brendan);
    else
        StringCopy(gSaveBlock2.playerName, gText_ExpandedPlaceholder_May);
}
...
static void MainMenu_FormatSavegamePlayer(void)
{
    ...
    AddTextPrinterParameterized3(2, FONT_NORMAL, GetStringRightAlignXOffset(FONT_NORMAL, gSaveBlock2.playerName, 100), 17, sTextColor_MenuInfo, TEXT_SKIP_DRAW, gSaveBlock2.playerName);
}
...
{
    LoadBgTiles(bgId, GetWindowFrameTilesPal(gSaveBlock2.optionsWindowFrameType)->tiles, 0x120, tileOffset);
    LoadPalette(GetWindowFrameTilesPal(gSaveBlock2.optionsWindowFrameType)->pal, BG_PLTT_ID(2), PLTT_SIZE_4BPP);
}
...
static void NewGameBirchSpeech_ClearGenderWindow(u8 windowId, bool8 copyToVram)
{
    ...
    FillBgTilemapBufferRect(window->bg, 0, window->tilemapLeft + 255, window->tilemapTop + 255, window->width + 2, window->height + 2, 2);
...
static void NewGameBirchSpeech_ShowDialogueWindow(void)
{
    CallWindowFunction(0, NewGameBirchSpeech_CreateDialogueWindowBorder);
