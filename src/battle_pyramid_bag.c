...
void TryStoreHeldItemsInPyramidBag(void)
{
    ...
    memcpy(newItems, gSaveBlock2.frontier.pyramidBag.itemId[gSaveBlock2.frontier.lvlMode], PYRAMID_BAG_ITEMS_COUNT * sizeof(u16));
    memcpy(newQuantities, gSaveBlock2.frontier.pyramidBag.quantity[gSaveBlock2.frontier.lvlMode], PYRAMID_BAG_ITEMS_COUNT * sizeof(u8));
    for (i = 0; i < 3; i++)
    {
        ...
        if (heldItem != ITEM_NONE && !AddBagItem(heldItem, 1))
        {
            ...
            memcpy(gSaveBlock2.frontier.pyramidBag.itemId[gSaveBlock2.frontier.lvlMode], newItems, PYRAMID_BAG_ITEMS_COUNT * sizeof(u16));
            memcpy(gSaveBlock2.frontier.pyramidBag.quantity[gSaveBlock2.frontier.lvlMode], newQuantities, PYRAMID_BAG_ITEMS_COUNT * sizeof(u8));
            Free(newItems);
            Free(newQuantities);
...
static void LoadPyramidBagPalette(void)
{
    ...
    spritePalette.data = palPtr + (gSaveBlock2.frontier.lvlMode * 16);
