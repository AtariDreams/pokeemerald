...
static void AddHatchedMonToParty(u8 id)
{
    ...
    u16 metLevel;
    u8 metLocation;
    struct Pokemon *mon = &gPlayerParty.party[id];

    CreateHatchedMon(mon, &gEnemyParty.party[0]);
...
{
    u8 spriteId = 0;
    struct Pokemon *mon = &gPlayerParty.party[partyId];
    switch (state)
    {
    case 0:
        // Load mon sprite gfx
...
{
    SetMonData(&gPlayerParty.party[gSpecialVar_0x8004], MON_DATA_NICKNAME, gStringVar3);
    FreeMonSpritesGfx();
    Free(sEggHatchData);
...
static void CB2_EggHatch(void)
{
    ...
    case 5:
        ...
        GetMonNickname2(&gPlayerParty.party[sEggHatchData->eggPartyId], gStringVar1);
    ...
    case 10:
        ...
        {
        case 0: // Yes
            GetMonNickname2(&gPlayerParty.party[sEggHatchData->eggPartyId], gStringVar3);
            species = GetMonData(&gPlayerParty.party[sEggHatchData->eggPartyId], MON_DATA_SPECIES);
            gender = GetMonGender(&gPlayerParty.party[sEggHatchData->eggPartyId]);
            personality = GetMonData(&gPlayerParty.party[sEggHatchData->eggPartyId], MON_DATA_PERSONALITY, 0);
...
u8 GetEggCyclesToSubtract(void)
{
    ...
    for (i = 0; i < gPlayerParty.count; i++)
    {
        ...
        {
            u8 ability = GetMonAbility(&gPlayerParty.party[i]);
