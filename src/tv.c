...
void PutNameRaterShowOnTheAir(void)
{
    ...
    {
        GetMonData(&gPlayerParty[gSpecialVar_0x8004], 2);
...
void TryPutSpotTheCutiesOnAir(struct Pokemon *pokemon, u8 ribbonMonDataIdx)
{
    ...
    if (sCurTVShowSlot != -1 && IsRecordMixShowAlreadySpawned(TVSHOW_CUTIES, FALSE) != TRUE)
    {
        ...
        GetMonData(pokemon, MON_DATA_NICKNAME);
...
void TryPutFrontierTVShowOnAir(u16 winStreak, u8 facilityAndMode)
{
    ...
    if (sCurTVShowSlot != -1)
    {
        ...
        case FRONTIER_SHOW_TOWER_LINK_MULTIS:
            show->frontier.species1 = GetMonData(&gSaveBlock1.playerParty[gSaveBlock2.frontier.selectedPartyMons[0] - 1], MON_DATA_SPECIES);
            show->frontier.species2 = GetMonData(&gSaveBlock1.playerParty[gSaveBlock2.frontier.selectedPartyMons[1] - 1], MON_DATA_SPECIES);
            break;
        }
        StorePlayerIdInRecordMixShow(show);
...
static void InterviewBefore_FanClubLetter(void)
{
    ...
    {
        StringCopy(gStringVar1, gSpeciesNames[GetMonData(&gPlayerParty[GetLeadMonIndex()], MON_DATA_SPECIES)]);
...
static bool8 IsPartyMonNicknamedOrNotEnglish(u8 monIdx)
{
    ...
    if (language == GAME_LANGUAGE && !StringCompare(gSpeciesNames[GetMonData(pokemon, MON_DATA_SPECIES)], gStringVar1))
        return FALSE;
...
void ChangePokemonNickname_CB(void)
{
    SetMonData(&gPlayerParty[gSpecialVar_0x8004], MON_DATA_NICKNAME, NULL);
