...
bool8 ScrCmd_copyobjectxytoperm(struct ScriptContext *ctx)
{
    ...
    TryOverrideObjectEventTemplateCoords(localId, gSaveBlock1.location.mapNum, gSaveBlock1.location.mapGroup);
    return FALSE;
}
...
bool8 ScrCmd_checkpartymove(struct ScriptContext *ctx)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
        ...
        if (!species)
            break;
        if (!GetMonData(&gPlayerParty[i], MON_DATA_IS_EGG, NULL) && MonKnowsMove(&gPlayerParty[i], moveId) == TRUE)
        {
