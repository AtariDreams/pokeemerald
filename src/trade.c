...
static bool8 BufferTradeParties(void)
{
    ...
    case 16:
        ...
        break;
    case 17:
        Trade_Memcpy(gBlockSendBuffer, gSaveBlock1.giftRibbons, sizeof(sTradeMenuData->giftRibbons));
        sTradeMenuData->bufferPartyState++;
        break;
    case 19:
        if (id == 0)
...
static u8 GetMonNicknameWidth(u8 *str, u8 whichParty, u8 monIdx)
{
    ...
    if (whichParty == TRADE_PLAYER)
        GetMonData(&gPlayerParty[monIdx], MON_DATA_NICKNAME);
    else
        GetMonData(&gEnemyParty[monIdx], MON_DATA_NICKNAME);
...
static void PrintPartyNicknamesForTradeMenu(u8 whichParty)
{
    ...
    for (i = 0; i < sTradeMenuData->partyCounts[whichParty]; i++)
    {
        GetMonData(&party[i], MON_DATA_NICKNAME);
        StringCopy_Nickname(str, nickname);
...
static void SaveTradeGiftRibbons(void)
{
    ...
    {
        if (gSaveBlock1.giftRibbons[i] == 0 && sTradeMenuData->giftRibbons[i] != 0)
        {
            if (sTradeMenuData->giftRibbons[i] < 64)
                gSaveBlock1.giftRibbons[i] = sTradeMenuData->giftRibbons[i];
        }
    }
}
...
static void CB2_InGameTrade(void)
{
    ...
    case 0:
        ...
        GetMonData(&gEnemyParty[0], MON_DATA_OT_NAME);
...
static void BufferTradeSceneStrings(void)
{
    ...
    if (sTradeData->isLinkTrade)
    {
        ...
        GetMonData(&gEnemyParty[gSelectedTradeMonPositions[TRADE_PARTNER] % PARTY_SIZE], MON_DATA_NICKNAME);
        StringCopy_Nickname(gStringVar3, name);
        GetMonData(&gPlayerParty[gSelectedTradeMonPositions[TRADE_PLAYER]], MON_DATA_NICKNAME);
    ...
    else
    {
        ...
        GetMonData(&gPlayerParty[gSpecialVar_0x8005], MON_DATA_NICKNAME);
        StringCopy_Nickname(gStringVar2, name);
    }
}
