...
static void GetChallengeStatus(void)
{
    ...
    switch (gSaveBlock2.frontier.challengeStatus)
    {
    case 0:
        break;
    case CHALLENGE_STATUS_SAVING:
        FrontierGamblerSetWonOrLost(FALSE);
        VarSet(VAR_TEMP_CHALLENGE_STATUS, gSaveBlock2.frontier.challengeStatus);
        break;
...
static void GetFrontierData(void)
{
    ...
    case FRONTIER_DATA_CHALLENGE_STATUS:
        gSpecialVar_Result = gSaveBlock2.frontier.challengeStatus;
        break;
    case FRONTIER_DATA_LVL_MODE:
        gSpecialVar_Result = gSaveBlock2.frontier.lvlMode;
        break;
    case FRONTIER_DATA_BATTLE_NUM:
        gSpecialVar_Result = gSaveBlock2.frontier.curChallengeBattleNum;
        break;
    case FRONTIER_DATA_PAUSED:
        gSpecialVar_Result = gSaveBlock2.frontier.challengePaused;
        break;
    ...
    case FRONTIER_DATA_RECORD_DISABLED:
        gSpecialVar_Result = gSaveBlock2.frontier.disableRecordBattle;
        break;
    case FRONTIER_DATA_HEARD_BRAIN_SPEECH:
        gSpecialVar_Result = gSaveBlock2.frontier.battledBrainFlags & sBattledBrainBitFlags[facility][hasSymbol];
        break;
    }
}
...
static void SetFrontierData(void)
{
    ...
    {
    case FRONTIER_DATA_CHALLENGE_STATUS:
        gSaveBlock2.frontier.challengeStatus = gSpecialVar_0x8006;
        break;
    case FRONTIER_DATA_LVL_MODE:
        gSaveBlock2.frontier.lvlMode = gSpecialVar_0x8006;
        break;
    case FRONTIER_DATA_BATTLE_NUM:
        gSaveBlock2.frontier.curChallengeBattleNum = gSpecialVar_0x8006;
        break;
    case FRONTIER_DATA_PAUSED:
        gSaveBlock2.frontier.challengePaused = gSpecialVar_0x8006;
        break;
    case FRONTIER_DATA_SELECTED_MON_ORDER:
        ...
        break;
    case FRONTIER_DATA_RECORD_DISABLED:
        gSaveBlock2.frontier.disableRecordBattle = gSpecialVar_0x8006;
        break;
    case FRONTIER_DATA_HEARD_BRAIN_SPEECH:
        gSaveBlock2.frontier.battledBrainFlags |= sBattledBrainBitFlags[facility][hasSymbol];
        break;
    }
}
...
static void SetSelectedPartyOrder(void)
{
    ...
    for (i = 0; i < gSpecialVar_0x8005; i++)
        gSelectedOrderFromParty[i] = gSaveBlock2.frontier.selectedPartyMons[i];
...
static void SaveSelectedParty(void)
{
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++)
    {
        u16 monId = gSaveBlock2.frontier.selectedPartyMons[i] - 1;
        if (monId < PARTY_SIZE)
            gSaveBlock1.playerParty[gSaveBlock2.frontier.selectedPartyMons[i] - 1] = gPlayerParty.party[i];
    }
}
...
{
    if (gSaveBlock2.frontier.winStreakActiveFlags & challenge)
        return TRUE;
    else
        return FALSE;
}
...
{
    u16 num = gSaveBlock2.frontier.towerRecordWinStreaks[battleMode][lvlMode];
...
{
    u16 winStreak = gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode];
    if (winStreak > MAX_STREAK)
        return MAX_STREAK;
    else
        return winStreak;
...
static void ShowDomeResultsWindow(u8 battleMode)
{
    ...
    PrintTwoStrings(gText_Record, gText_ClearStreak, gSaveBlock2.frontier.domeRecordWinStreaks[battleMode][FRONTIER_LVL_50], 64, 121, 49);
    PrintTwoStrings(gText_Total, gText_Championships, gSaveBlock2.frontier.domeTotalChampionships[battleMode][FRONTIER_LVL_50], 64, 112, 65);
    DomePrintPrevOrCurrentStreak(battleMode, FRONTIER_LVL_OPEN, 64, 121, 97);
    PrintTwoStrings(gText_Record, gText_ClearStreak, gSaveBlock2.frontier.domeRecordWinStreaks[battleMode][FRONTIER_LVL_OPEN], 64, 121, 113);
    PrintTwoStrings(gText_Total, gText_Championships, gSaveBlock2.frontier.domeTotalChampionships[battleMode][FRONTIER_LVL_OPEN], 64, 112, 129);
    PutWindowTilemap(gRecordsWindowId);
    CopyWindowToVram(gRecordsWindowId, COPYWIN_FULL);
}
...
{
    u16 num = gSaveBlock2.frontier.palaceRecordWinStreaks[battleMode][lvlMode];
    PalacePrintStreak(gText_Record, num, x1, x2, y);
}

static u16 PalaceGetWinStreak(u8 battleMode, u8 lvlMode)
{
    u16 winStreak = gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode];
    if (winStreak > MAX_STREAK)
        return MAX_STREAK;
    else
        return winStreak;
...
{
    u16 winStreak = gSaveBlock2.frontier.pikeWinStreaks[lvlMode];
    if (winStreak > MAX_STREAK)
        return MAX_STREAK;
    else
        return winStreak;
...
static void ShowPikeResultsWindow(void)
{
    ...
    PikePrintCleared(gText_Record, gText_RoomsCleared, gSaveBlock2.frontier.pikeRecordStreaks[FRONTIER_LVL_50], 64, 114, 49);
    PikePrintCleared(gText_Total, gText_TimesCleared, gSaveBlock2.frontier.pikeTotalStreaks[FRONTIER_LVL_50], 64, 114, 65);
    PikePrintPrevOrCurrentStreak(FRONTIER_LVL_OPEN, 64, 114, 97);
    PikePrintCleared(gText_Record, gText_RoomsCleared, gSaveBlock2.frontier.pikeRecordStreaks[FRONTIER_LVL_OPEN], 64, 114, 113);
    PikePrintCleared(gText_Total, gText_TimesCleared, gSaveBlock2.frontier.pikeTotalStreaks[FRONTIER_LVL_OPEN], 64, 114, 129);
    PutWindowTilemap(gRecordsWindowId);
    CopyWindowToVram(gRecordsWindowId, COPYWIN_FULL);
}
...
{
    u16 num = gSaveBlock2.frontier.arenaRecordStreaks[lvlMode];
    ArenaPrintStreak(gText_Record, num, x1, x2, y);
}

static u16 ArenaGetWinStreak(u8 lvlMode)
{
    u16 winStreak = gSaveBlock2.frontier.arenaWinStreaks[lvlMode];
    if (winStreak > MAX_STREAK)
        return MAX_STREAK;
    else
        return winStreak;
...
{
    u16 num1 = gSaveBlock2.frontier.factoryRecordWinStreaks[battleMode][lvlMode];
    u16 num2 = gSaveBlock2.frontier.factoryRecordRentsCount[battleMode][lvlMode];
    FactoryPrintStreak(gText_Record, num1, num2, x1, x2, x3, y);
}

static u16 FactoryGetWinStreak(u8 battleMode, u8 lvlMode)
{
    u16 winStreak = gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode];
    if (winStreak > MAX_STREAK)
        return MAX_STREAK;
    else
        return winStreak;
...
{
    u16 rents = gSaveBlock2.frontier.factoryRentsCount[battleMode][lvlMode];
    if (rents > MAX_STREAK)
        return MAX_STREAK;
    else
        return rents;
...
{
    u16 num = gSaveBlock2.frontier.pyramidRecordStreaks[lvlMode];
    PyramidPrintStreak(gText_Record, num, x1, x2, y);
}

static u16 PyramidGetWinStreak(u8 lvlMode)
{
    u16 winStreak = gSaveBlock2.frontier.pyramidWinStreaks[lvlMode];
    if (winStreak > MAX_STREAK)
        return MAX_STREAK;
    else
        return winStreak;
...
static void ShowLinkContestResultsWindow(void)
{
    ...
    for (i = 0; i < CONTEST_CATEGORIES_COUNT; i++)
    {
        ...
        {
            ConvertIntToDecimalStringN(gStringVar4, gSaveBlock2.contestLinkResults[i][j], STR_CONV_MODE_RIGHT_ALIGN, 4);
...
{
    u8 name[32];
    s32 lvlMode = gSaveBlock2.frontier.lvlMode;
    ...
    {
    case FRONTIER_FACILITY_TOWER:
        if (gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode] > gSaveBlock2.frontier.towerRecordWinStreaks[battleMode][lvlMode])
        {
            gSaveBlock2.frontier.towerRecordWinStreaks[battleMode][lvlMode] = gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode];
            if (battleMode == FRONTIER_MODE_LINK_MULTIS)
            {
                StringCopy(name, gLinkPlayers[gBattleScripting.multiplayerId ^ 1].name);
                StripExtCtrlCodes(name);
                StringCopy(gSaveBlock2.frontier.opponentNames[lvlMode], name);
                SetTrainerId(gLinkPlayers[gBattleScripting.multiplayerId ^ 1].trainerId, gSaveBlock2.frontier.opponentTrainerIds[lvlMode]);
            }
            if (gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode] > 1
                && ShouldAirFrontierTVShow())
            {
                switch (battleMode)
                {
                case FRONTIER_MODE_SINGLES:
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_TOWER_SINGLES);
                    break;
                case FRONTIER_MODE_DOUBLES:
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_TOWER_DOUBLES);
                    break;
                case FRONTIER_MODE_MULTIS:
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_TOWER_MULTIS);
                    break;
                case FRONTIER_MODE_LINK_MULTIS:
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_TOWER_LINK_MULTIS);
                    break;
                }
            }
        }
        break;
    case FRONTIER_FACILITY_DOME:
        if (gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode] > gSaveBlock2.frontier.domeRecordWinStreaks[battleMode][lvlMode])
        {
            gSaveBlock2.frontier.domeRecordWinStreaks[battleMode][lvlMode] = gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode];
            if (gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode] > 1
                && ShouldAirFrontierTVShow())
            {
                if (battleMode == FRONTIER_MODE_SINGLES)
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_DOME_SINGLES);
                else
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_DOME_DOUBLES);
            }
        }
        break;
    case FRONTIER_FACILITY_PALACE:
        if (gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode] > gSaveBlock2.frontier.palaceRecordWinStreaks[battleMode][lvlMode])
        {
            gSaveBlock2.frontier.palaceRecordWinStreaks[battleMode][lvlMode] = gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode];
            if (gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode] > 1
                && ShouldAirFrontierTVShow())
            {
                if (battleMode == FRONTIER_MODE_SINGLES)
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_PALACE_SINGLES);
                else
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_PALACE_DOUBLES);
            }
        }
        break;
    case FRONTIER_FACILITY_ARENA:
        if (gSaveBlock2.frontier.arenaWinStreaks[lvlMode] > gSaveBlock2.frontier.arenaRecordStreaks[lvlMode])
        {
            gSaveBlock2.frontier.arenaRecordStreaks[lvlMode] = gSaveBlock2.frontier.arenaWinStreaks[lvlMode];
            if (gSaveBlock2.frontier.arenaWinStreaks[lvlMode] > 1
                && ShouldAirFrontierTVShow())
            {
                TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.arenaWinStreaks[lvlMode], FRONTIER_SHOW_ARENA);
            }
        }
        break;
    case FRONTIER_FACILITY_FACTORY:
        if (gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode] > gSaveBlock2.frontier.factoryRecordWinStreaks[battleMode][lvlMode])
        {
            gSaveBlock2.frontier.factoryRecordWinStreaks[battleMode][lvlMode] = gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode];
            gSaveBlock2.frontier.factoryRecordRentsCount[battleMode][lvlMode] = gSaveBlock2.frontier.factoryRentsCount[battleMode][lvlMode];
            if (gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode] > 1
                && ShouldAirFrontierTVShow())
            {
                if (battleMode == FRONTIER_MODE_SINGLES)
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_FACTORY_SINGLES);
                else
                    TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode], FRONTIER_SHOW_FACTORY_DOUBLES);
            }
        }
        break;
    case FRONTIER_FACILITY_PIKE:
        if (gSaveBlock2.frontier.pikeWinStreaks[lvlMode] > gSaveBlock2.frontier.pikeRecordStreaks[lvlMode])
        {
            gSaveBlock2.frontier.pikeRecordStreaks[lvlMode] = gSaveBlock2.frontier.pikeWinStreaks[lvlMode];
            if (gSaveBlock2.frontier.pikeWinStreaks[lvlMode] > 1
                && ShouldAirFrontierTVShow())
            {
                TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.pikeWinStreaks[lvlMode], FRONTIER_SHOW_PIKE);
            }
        }
        break;
    case FRONTIER_FACILITY_PYRAMID:
        if (gSaveBlock2.frontier.pyramidWinStreaks[lvlMode] > gSaveBlock2.frontier.pyramidRecordStreaks[lvlMode])
        {
            gSaveBlock2.frontier.pyramidRecordStreaks[lvlMode] = gSaveBlock2.frontier.pyramidWinStreaks[lvlMode];
            if (gSaveBlock2.frontier.pyramidWinStreaks[lvlMode] > 1
                && ShouldAirFrontierTVShow())
            {
                TryPutFrontierTVShowOnAir(gSaveBlock2.frontier.pyramidWinStreaks[lvlMode], FRONTIER_SHOW_PYRAMID);
            }
        }
        break;
    }
    ...
    case FRONTIER_PLAYER_LOST_TEXT:
        ...
        else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE)
        {
            if (gBattleTypeFlags & BATTLE_TYPE_RECORDED)
                ...
                FrontierSpeechToString(gSaveBlock2.frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND].speechWon);
        }
        else
        {
            if (gBattleTypeFlags & BATTLE_TYPE_RECORDED)
                ...
                FrontierSpeechToString(gSaveBlock2.apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].speechWon);
        }
        break;
    case FRONTIER_PLAYER_WON_TEXT:
        ...
        else if (trainerId < TRAINER_RECORD_MIXING_APPRENTICE)
        {
            if (gBattleTypeFlags & BATTLE_TYPE_RECORDED)
                ...
                FrontierSpeechToString(gSaveBlock2.frontier.towerRecords[trainerId - TRAINER_RECORD_MIXING_FRIEND].speechLost);
        }
        else
        {
            if (gBattleTypeFlags & BATTLE_TYPE_RECORDED)
            ...
            {
                trainerId = gSaveBlock2.apprentices[trainerId - TRAINER_RECORD_MIXING_APPRENTICE].id;
                FrontierSpeechToString(gApprentices[trainerId].speechLost);
            }
        }
        break;
...
void ResetWinStreaks(void)
{
    ...
    gSaveBlock2.frontier.winStreakActiveFlags = 0;
    for (battleMode = 0; battleMode < FRONTIER_MODE_COUNT; battleMode++)
    {
        for (lvlMode = 0; lvlMode < FRONTIER_LVL_TENT; lvlMode++)
        {
            ...
            {
                gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode] = 0;
                gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode] = 0;
                gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode] = 0;
            }
            if (battleMode == FRONTIER_MODE_SINGLES)
            {
                gSaveBlock2.frontier.arenaWinStreaks[lvlMode] = 0;
                gSaveBlock2.frontier.pikeWinStreaks[lvlMode] = 0;
                gSaveBlock2.frontier.pyramidWinStreaks[lvlMode] = 0;
            }
        }
    }
    if (gSaveBlock2.frontier.challengeStatus != 0)
...
u32 GetCurrentFacilityWinStreak(void)
{
    ...
    {
    case FRONTIER_FACILITY_TOWER:
        return gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode];
    case FRONTIER_FACILITY_DOME:
        return gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode];
    case FRONTIER_FACILITY_PALACE:
        return gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode];
    case FRONTIER_FACILITY_ARENA:
        return gSaveBlock2.frontier.arenaWinStreaks[lvlMode];
    case FRONTIER_FACILITY_FACTORY:
        return gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode];
    case FRONTIER_FACILITY_PIKE:
        return gSaveBlock2.frontier.pikeWinStreaks[lvlMode];
    case FRONTIER_FACILITY_PYRAMID:
        return gSaveBlock2.frontier.pyramidWinStreaks[lvlMode];
    default:
        return 0;
    }
}
...
static void GiveBattlePoints(void)
{
    ...
    case FRONTIER_FACILITY_TOWER:
        ...
        break;
    case FRONTIER_FACILITY_DOME:
        challengeNum = gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode];
        break;
    case FRONTIER_FACILITY_PALACE:
        challengeNum = gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode] / FRONTIER_STAGES_PER_CHALLENGE;
        break;
    case FRONTIER_FACILITY_ARENA:
        challengeNum = gSaveBlock2.frontier.arenaWinStreaks[lvlMode] / FRONTIER_STAGES_PER_CHALLENGE;
        break;
    case FRONTIER_FACILITY_FACTORY:
        challengeNum = gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode] / FRONTIER_STAGES_PER_CHALLENGE;
        break;
    case FRONTIER_FACILITY_PIKE:
        challengeNum = gSaveBlock2.frontier.pikeWinStreaks[lvlMode] / NUM_PIKE_ROOMS;
        break;
    case FRONTIER_FACILITY_PYRAMID:
        challengeNum = gSaveBlock2.frontier.pyramidWinStreaks[lvlMode] / FRONTIER_STAGES_PER_CHALLENGE;
        break;
    }
    ...
    if (gTrainerBattleOpponent_A == TRAINER_FRONTIER_BRAIN)
        points += 10;
    gSaveBlock2.frontier.battlePoints += points;
    ConvertIntToDecimalStringN(gStringVar1, points, STR_CONV_MODE_LEFT_ALIGN, 2);
    if (gSaveBlock2.frontier.battlePoints > MAX_BATTLE_FRONTIER_POINTS)
        gSaveBlock2.frontier.battlePoints = MAX_BATTLE_FRONTIER_POINTS;

    points = gSaveBlock2.frontier.cardBattlePoints;
    points += sBattlePointAwards[challengeNum][facility][battleMode];
    ...
    if (points > 0xFFFF)
        points = 0xFFFF;
    gSaveBlock2.frontier.cardBattlePoints = points;
}
...
static void CheckPartyIneligibility(void)
{
    ...
    do
    {
        ...
        do
        {
            u16 species = GetMonData(&gPlayerParty.party[monId], MON_DATA_SPECIES_OR_EGG);
            u16 heldItem = GetMonData(&gPlayerParty.party[monId], MON_DATA_HELD_ITEM);
            u8 level = GetMonData(&gPlayerParty.party[monId], MON_DATA_LEVEL);
            u16 hp = GetMonData(&gPlayerParty.party[monId], MON_DATA_HP);
    ...
    else
    {
        ...
        gSaveBlock2.frontier.lvlMode = gSpecialVar_Result;
    }
...
static void IncrementWinStreak(void)
{
    ...
    {
    case FRONTIER_FACILITY_TOWER:
        if (gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode] < MAX_STREAK)
        {
            gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode]++;
            if (battleMode == FRONTIER_MODE_SINGLES)
            {
                SetGameStat(GAME_STAT_BATTLE_TOWER_SINGLES_STREAK, gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode]);
                gSaveBlock2.frontier.towerSinglesStreak = gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode];
            }
        }
        break;
    case FRONTIER_FACILITY_DOME:
        if (gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode] < MAX_STREAK)
            gSaveBlock2.frontier.domeWinStreaks[battleMode][lvlMode]++;
        if (gSaveBlock2.frontier.domeTotalChampionships[battleMode][lvlMode] < MAX_STREAK)
            gSaveBlock2.frontier.domeTotalChampionships[battleMode][lvlMode]++;
        break;
    case FRONTIER_FACILITY_PALACE:
        if (gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode] < MAX_STREAK)
            gSaveBlock2.frontier.palaceWinStreaks[battleMode][lvlMode]++;
        break;
    case FRONTIER_FACILITY_ARENA:
        if (gSaveBlock2.frontier.arenaWinStreaks[lvlMode] < MAX_STREAK)
            gSaveBlock2.frontier.arenaWinStreaks[lvlMode]++;
        break;
    case FRONTIER_FACILITY_FACTORY:
        if (gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode] < MAX_STREAK)
            gSaveBlock2.frontier.factoryWinStreaks[battleMode][lvlMode]++;
        break;
    case FRONTIER_FACILITY_PIKE:
        if (gSaveBlock2.frontier.pikeWinStreaks[lvlMode] < MAX_STREAK)
            gSaveBlock2.frontier.pikeWinStreaks[lvlMode]++;
        break;
    case FRONTIER_FACILITY_PYRAMID:
        if (gSaveBlock2.frontier.pyramidWinStreaks[lvlMode] < MAX_STREAK)
            gSaveBlock2.frontier.pyramidWinStreaks[lvlMode]++;
        break;
    }
}
...
static void RestoreHeldItems(void)
{
    ...
    for (i = 0; i < MAX_FRONTIER_PARTY_SIZE; i++)
    {
        if (gSaveBlock2.frontier.selectedPartyMons[i] != 0)
        {
            ...
            SetMonData(&gPlayerParty.party[i], MON_DATA_HELD_ITEM, &item);
        }
    }
}
            ...
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
                for (k = 0; k < MAX_MON_MOVES; k++)
                {
                    if (GetMonData(&gSaveBlock1.playerParty[gSaveBlock2.frontier.selectedPartyMons[i] - 1], MON_DATA_MOVE1 + k, NULL)
                        == GetMonData(&gPlayerParty.party[i], MON_DATA_MOVE1 + j, NULL))
                        break;
                }
                if (k == MAX_MON_MOVES)
                    SetMonMoveSlot(&gPlayerParty.party[i], MOVE_SKETCH, j);
            }
            gSaveBlock1.playerParty[gSaveBlock2.frontier.selectedPartyMons[i] - 1] = gPlayerParty.party[i];
        }
    }
...
static void Fill1PRecords(struct RankingHall1P *dst, s32 hallFacilityId, s32 lvlMode)
{
    ...
    for (i = 0; i < HALL_RECORDS_COUNT; i++)
        record1P[i] = gSaveBlock2.hallRecords1P[hallFacilityId][lvlMode][i];
...
static void Fill2PRecords(struct RankingHall2P *dst, s32 lvlMode)
{
    ...
    for (i = 0; i < HALL_RECORDS_COUNT; i++)
        record2P[i] = gSaveBlock2.hallRecords2P[lvlMode][i];
...
static void PrintHallRecords(s32 hallFacilityId, s32 lvlMode)
{
    ...
    {
        gSaveBlock2.frontier.opponentNames[0][PLAYER_NAME_LENGTH] = EOS;
        gSaveBlock2.frontier.opponentNames[1][PLAYER_NAME_LENGTH] = EOS;
...
void ClearRankingHallRecords(void)
{
    ...
    for (i = 0; i < HALL_FACILITIES_COUNT; i++)
    {
        for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++)
        {
            ...
            {
                CopyTrainerId(gSaveBlock2.hallRecords1P[i][j][k].id, ZERO);
                gSaveBlock2.hallRecords1P[i][j][k].name[0] = EOS;
                gSaveBlock2.hallRecords1P[i][j][k].winStreak = 0;
            }
        }
    }
    ...
    for (j = 0; j < FRONTIER_LVL_MODE_COUNT; j++)
    {
        for (k = 0; k < HALL_RECORDS_COUNT; k++)
        {
            CopyTrainerId(gSaveBlock2.hallRecords2P[j][k].id1, ZERO);
            CopyTrainerId(gSaveBlock2.hallRecords2P[j][k].id2, ZERO);
            gSaveBlock2.hallRecords2P[j][k].name1[0] = EOS;
            gSaveBlock2.hallRecords2P[j][k].name2[0] = EOS;
            gSaveBlock2.hallRecords2P[j][k].winStreak = 0;
        }
    }
}
...
void SaveGameFrontier(void)
{
    ...
    for (i = 0; i < PARTY_SIZE; i++)
        monsParty[i] = gPlayerParty.party[i];

    i = gPlayerParty.count;
    LoadPlayerParty();
    SetContinueGameWarpStatusToDynamicWarp();
    TrySavingData(SAVE_LINK);
    ClearContinueGameWarpStatus2();
    gPlayerParty.count = i;

    for (i = 0; i < PARTY_SIZE; i++)
        gPlayerParty.party[i] = monsParty[i];
...
void CreateFrontierBrainPokemon(void)
{
    ...
    for (i = 0; i < FRONTIER_PARTY_SIZE; selectedMonBits >>= 1, i++)
    {
        ...
        CreateMon(&gEnemyParty.party[monPartyId],
                  sFrontierBrainsMons[facility][symbol][i].species,
                  monLevel,
                  sFrontierBrainsMons[facility][symbol][i].fixedIV,
                  TRUE, j,
                  OT_ID_PRESET, FRONTIER_BRAIN_OTID);
        SetMonData(&gEnemyParty.party[monPartyId], MON_DATA_HELD_ITEM, &sFrontierBrainsMons[facility][symbol][i].heldItem);
        for (j = 0; j < NUM_STATS; j++)
            SetMonData(&gEnemyParty.party[monPartyId], MON_DATA_HP_EV + j, &sFrontierBrainsMons[facility][symbol][i].evs[j]);
        friendship = MAX_FRIENDSHIP;
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            SetMonMoveSlot(&gEnemyParty.party[monPartyId], sFrontierBrainsMons[facility][symbol][i].moves[j], j);
            if (sFrontierBrainsMons[facility][symbol][i].moves[j] == MOVE_FRUSTRATION)
                friendship = 0;
        }
        SetMonData(&gEnemyParty.party[monPartyId], MON_DATA_FRIENDSHIP, &friendship);
        CalculateMonStats(&gEnemyParty.party[monPartyId]);
        monPartyId++;
    }
}
