    ...
    case 19:
        {
            ...
            species = GetMonData(&gEnemyParty.party[gBattlerPartyIndexes[opponentBattler]], MON_DATA_SPECIES);
    ...
    if (battler < gBattlersCount)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            if (!gBattleSpritesDataPtr->battlerData[battler].behindSubstitute)
                BattleLoadOpponentMonSpriteGfx(&gEnemyParty.party[gBattlerPartyIndexes[battler]], battler);
            else
            ...
            DecompressTrainerBackPic(gSaveBlock2.playerGender, battler);
            ...
            BattleLoadPlayerMonSpriteGfx(&gPlayerParty.party[gBattlerPartyIndexes[battler]], battler);
        else
            BattleLoadSubstituteOrMonSpriteGfx(battler, FALSE);
...
static void CreateBattlerSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
        {
            if (GetMonData(&gEnemyParty.party[gBattlerPartyIndexes[battler]], MON_DATA_HP) == 0)
                return;

            SetMultiuseSpriteTemplateToPokemon(GetMonData(&gEnemyParty.party[gBattlerPartyIndexes[battler]], MON_DATA_SPECIES), GetBattlerPosition(battler));
            ...
            gSprites[gBattlerSpriteIds[battler]].data[2] = GetMonData(&gEnemyParty.party[gBattlerPartyIndexes[battler]], MON_DATA_SPECIES);
        ...
        {
            SetMultiuseSpriteTemplateToTrainerBack(gSaveBlock2.playerGender, GetBattlerPosition(B_POSITION_PLAYER_LEFT));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, 0x50,
                                                (8 - gTrainerBackPicCoords[gSaveBlock2.playerGender].size) * 4 + 80,
        ...
        {
            if (GetMonData(&gPlayerParty.party[gBattlerPartyIndexes[battler]], MON_DATA_HP) == 0)
                return;

            SetMultiuseSpriteTemplateToPokemon(GetMonData(&gPlayerParty.party[gBattlerPartyIndexes[battler]], MON_DATA_SPECIES), GetBattlerPosition(battler));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, GetBattlerSpriteCoord(battler, BATTLER_COORD_X_2), posY, GetBattlerSpriteSubpriority(battler));
            gSprites[gBattlerSpriteIds[battler]].oam.paletteNum = battler;
            gSprites[gBattlerSpriteIds[battler]].callback = SpriteCallbackDummy;
            gSprites[gBattlerSpriteIds[battler]].data[0] = battler;
            gSprites[gBattlerSpriteIds[battler]].data[2] = GetMonData(&gPlayerParty.party[gBattlerPartyIndexes[battler]], MON_DATA_SPECIES);
...
static void CreateHealthboxSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        if (GetBattlerSide(battler) != B_SIDE_PLAYER)
            UpdateHealthboxAttribute(gHealthboxSpriteIds[battler], &gEnemyParty.party[gBattlerPartyIndexes[battler]], HEALTHBOX_ALL);
        else if (gBattleTypeFlags & BATTLE_TYPE_SAFARI)
            UpdateHealthboxAttribute(gHealthboxSpriteIds[battler], &gPlayerParty.party[gBattlerPartyIndexes[battler]], HEALTHBOX_SAFARI_ALL_TEXT);
        else
            UpdateHealthboxAttribute(gHealthboxSpriteIds[battler], &gPlayerParty.party[gBattlerPartyIndexes[battler]], HEALTHBOX_ALL);
