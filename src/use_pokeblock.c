...
static void AskUsePokeblock(void)
{
    ...
    GetMonData(&gPlayerParty.party[GetPartyIdFromSelectionId(sMenu->info.curSelection)], MON_DATA_NICKNAME, stringBuffer);
...
static void UpdateMonInfoText(u16 loadId, bool8 firstPrint)
{
    ...
    if (sMenu->info.curSelection != sMenu->info.numSelections - 1)
    {
        ...
        nature = GetNature(&gPlayerParty.party[partyIndex]);
...
{
    u8 sheen = GetMonData(&gPlayerParty.party[monIndex], MON_DATA_SHEEN);
