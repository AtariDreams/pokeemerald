...
static bool8 SetupContestGraphics(u8 *stateVar)
{
    ...
    case 5:
        LoadCompressedPalette(gContestInterfaceAudiencePalette, BG_PLTT_OFFSET, BG_PLTT_SIZE);
        CpuFastCopy(&gPlttBufferUnfaded[BG_PLTT_ID(8)], tempPalette1, PLTT_SIZE_4BPP);
        CpuFastCopy(&gPlttBufferUnfaded[BG_PLTT_ID(5 + gContestPlayerMonIndex)], tempPalette2, PLTT_SIZE_4BPP);
        CpuFastCopy(tempPalette2, &gPlttBufferUnfaded[BG_PLTT_ID(8)], PLTT_SIZE_4BPP);
        CpuFastCopy(tempPalette1, &gPlttBufferUnfaded[BG_PLTT_ID(5 + gContestPlayerMonIndex)], PLTT_SIZE_4BPP);
        CpuFastCopy(gPlttBufferUnfaded, eContestTempSave.cachedWindowPalettes, sizeof(eContestTempSave.cachedWindowPalettes));
        LoadContestPalettes();
        break;
    case 6:
        ...
        CreateInvisibleBattleTargetSprite(); // Register an invisible actor instead of an enemy Pokémon.
...
static void Task_DisplayAppealNumberText(u8 taskId)
{
    if (gTasks[taskId].data[0] == 0)
    {
        ...
        gBattle_BG2_Y = 0;
        //ContestDebugDoPrint();
        CpuFastCopy(gPlttBufferUnfaded, eContestTempSave.cachedPlttBufferUnfaded, PLTT_SIZE);
...
static void Task_HideMoveSelectScreen(u8 taskId)
{
    ...
    DmaCopy32(3, gPlttBufferFaded, eContestTempSave.cachedPlttBufferFaded, sizeof(eContestTempSave.cachedPlttBufferFaded));
...
{
    DmaCopy32(3, eContestTempSave.cachedPlttBufferUnfaded, gPlttBufferUnfaded, PLTT_SIZE);
    gTasks[taskId].data[0] = 0;
...
void CreateContestMonFromParty(u8 partyIndex)
{
    ...
    gContestMons[gContestPlayerMonIndex].species = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_SPECIES);
    GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_NICKNAME, name);
    StringGet_Nickname(name);
    if (gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)
    {
        StripMonNameForLinkContest(name, GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_LANGUAGE));
    }
    ...
    gContestMons[gContestPlayerMonIndex].cool = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_COOL);
    gContestMons[gContestPlayerMonIndex].beauty = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_BEAUTY);
    gContestMons[gContestPlayerMonIndex].cute = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_CUTE);
    gContestMons[gContestPlayerMonIndex].smart = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_SMART);
    gContestMons[gContestPlayerMonIndex].tough = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_TOUGH);
    gContestMons[gContestPlayerMonIndex].sheen = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_SHEEN);
    gContestMons[gContestPlayerMonIndex].moves[0] = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_MOVE1);
    gContestMons[gContestPlayerMonIndex].moves[1] = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_MOVE2);
    gContestMons[gContestPlayerMonIndex].moves[2] = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_MOVE3);
    gContestMons[gContestPlayerMonIndex].moves[3] = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_MOVE4);
    gContestMons[gContestPlayerMonIndex].personality = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_PERSONALITY);
    gContestMons[gContestPlayerMonIndex].otId = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_OT_ID);

    heldItem = GetMonData(&gPlayerParty.party[partyIndex], MON_DATA_HELD_ITEM);
...
void SaveLinkContestResults(void)
{
    if ((gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK))
    {
        gSaveBlock2.contestLinkResults[gSpecialVar_ContestCategory][gContestFinalStandings[gContestPlayerMonIndex]] =
        ((gSaveBlock2.contestLinkResults[gSpecialVar_ContestCategory][gContestFinalStandings[gContestPlayerMonIndex]] + 1) > 9999) ? 9999 :
        (gSaveBlock2.contestLinkResults[gSpecialVar_ContestCategory][gContestFinalStandings[gContestPlayerMonIndex]] + 1);
...
void ResetContestLinkResults(void)
{
    ...
    for(i = 0; i < CONTEST_CATEGORIES_COUNT; i++)
        for(j = 0; j < CONTESTANT_COUNT; j++)
            gSaveBlock2.contestLinkResults[i][j] = 0;
}
...
bool8 SaveContestWinner(u8 rank)
{
    ...
    if (rank != CONTEST_SAVE_FOR_ARTIST)
    {
        ...
        gSaveBlock1.contestWinners[id].personality = gContestMons[i].personality;
        gSaveBlock1.contestWinners[id].species = gContestMons[i].species;
        gSaveBlock1.contestWinners[id].trainerId = gContestMons[i].otId;
        StringCopy(gSaveBlock1.contestWinners[id].monName, gContestMons[i].nickname);
        StringCopy(gSaveBlock1.contestWinners[id].trainerName, gContestMons[i].trainerName);
        if(gLinkContestFlags & LINK_CONTEST_FLAG_IS_LINK)
            gSaveBlock1.contestWinners[id].contestRank = CONTEST_RANK_LINK;
        else
            gSaveBlock1.contestWinners[id].contestRank = gSpecialVar_ContestRank;

        if (rank != CONTEST_SAVE_FOR_MUSEUM)
            gSaveBlock1.contestWinners[id].contestCategory = gSpecialVar_ContestCategory;
        else
            gSaveBlock1.contestWinners[id].contestCategory = captionId;
    }
    else
    {
