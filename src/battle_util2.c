...
void AllocateBattleResources(void)
{
    ...
    {
        CreateSecretBaseEnemyParty(&gSaveBlock1.secretBases[VarGet(VAR_CURRENT_SECRET_BASE)]);
    }
}
...
void AdjustFriendshipOnBattleFaint(u8 battlerId)
{
    ...
    if (gBattleMons[opposingBattlerId].level > gBattleMons[battlerId].level)
    {
        if (gBattleMons[opposingBattlerId].level - gBattleMons[battlerId].level > 29)
            AdjustFriendship(&gPlayerParty.party[gBattlerPartyIndexes[battlerId]], FRIENDSHIP_EVENT_FAINT_LARGE);
        else
            AdjustFriendship(&gPlayerParty.party[gBattlerPartyIndexes[battlerId]], FRIENDSHIP_EVENT_FAINT_SMALL);
    }
    else
    {
        AdjustFriendship(&gPlayerParty.party[gBattlerPartyIndexes[battlerId]], FRIENDSHIP_EVENT_FAINT_SMALL);
