...
void GetPokeblockFeederWithinRange(void)
{
    ...
    for (i = 0; i < NUM_POKEBLOCK_FEEDERS; i++)
    {
        if (gSaveBlock1.location.mapNum == sPokeblockFeeders[i].mapNum)
        {
            ...
            if ((abs(x - sPokeblockFeeders[i].x) + abs(y - sPokeblockFeeders[i].y)) <= 5)
            {
                gSpecialVar_Result = i;
                return;
            }
...
void SafariZoneActivatePokeblockFeeder(u8 pkblId)
{
    ...
    for (i = 0; i < NUM_POKEBLOCK_FEEDERS; i++)
    {
         ...
         && sPokeblockFeeders[i].y == 0)
        {
            ...
            GetXYCoordsOneStepInFrontOfPlayer(&x, &y);
            sPokeblockFeeders[i].mapNum = gSaveBlock1.location.mapNum;
            sPokeblockFeeders[i].pokeblock = gSaveBlock1.pokeblocks[pkblId];
