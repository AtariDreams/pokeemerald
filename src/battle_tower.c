...
static void SetNextFacilityOpponent(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            SetBattleFacilityTrainerGfxId(gTrainerBattleOpponent_A, 0);
            if (gSaveBlock2.frontier.curChallengeBattleNum + 1 < 7)
                gSaveBlock2.frontier.trainerIds[gSaveBlock2.frontier.curChallengeBattleNum] = gTrainerBattleOpponent_A;
        }
    }
}
...
void PutNewBattleTowerRecord(struct EmeraldBattleTowerRecord *newRecordEm)
{
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++)
    {
        ...
        if (j == 4)
        {
            ...
            {
                #ifdef BUGFIX
                if (gSaveBlock2.frontier.towerRecords[i].name[k] != newRecord->name[k])
                    break;
                if (newRecord->name[k] == EOS)
                #else
                if (gSaveBlock2.frontier.towerRecords[i].name[j] != newRecord->name[j])
                    break;
    ...
    while (i != monCount)
    {
        ...
        for (j = 0; j < i + firstMonId; j++)
        {
            if (GetMonData(&gEnemyParty[j], MON_DATA_HELD_ITEM) != ITEM_NONE
             && GetMonData(&gEnemyParty[j], MON_DATA_HELD_ITEM) == gBattleFrontierHeldItems[gFacilityTrainerMons[monId].itemTableId])
                break;
        }
        if (j != i + firstMonId)
            continue;
...
static void FillFactoryFrontierTrainerParty(u16 trainerId, u8 firstMonId)
{
    ...
    if (trainerId < FRONTIER_TRAINERS_COUNT)
    {
        u8 lvlMode = gSaveBlock2.frontier.lvlMode; // Unused variable.
        u8 battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);
        u8 challengeNum = gSaveBlock2.frontier.towerWinStreaks[battleMode][FRONTIER_LVL_50] / 7;
        if (gSaveBlock2.frontier.curChallengeBattleNum < 6)
...
static void SaveTowerChallenge(void)
{
    ...
    s32 challengeNum = (signed)(gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode] / 7);
...
static void LoadMultiPartnerCandidatesData(void)
{
    ...
    lvlMode = gSaveBlock2.frontier.lvlMode;
    battleMode = VarGet(VAR_FRONTIER_BATTLE_MODE);
    challengeNum = gSaveBlock2.frontier.towerWinStreaks[battleMode][lvlMode] / 7;
    ...
    {
        if (gSaveBlock2.apprentices[i].lvlMode != 0
            && sApprenticeChallengeThreshold[gSaveBlock2.apprentices[i].numQuestions] / 7 <= challengeNum
            && gSaveBlock2.apprentices[i].lvlMode - 1 == lvlMode)
        {
            k = 0;
    ...
    for (i = 0; i < BATTLE_TOWER_RECORD_COUNT; i++)
    {
        ...
        if (gSaveBlock2.frontier.towerRecords[i].winStreak / 7 <= challengeNum
            && gSaveBlock2.frontier.towerRecords[i].lvlMode == lvlMode
            && recordHasData
            && gSaveBlock2.frontier.towerRecords[i].checksum == checksum)
        {
...
static void SetTowerInterviewData(void)
{
    ...
    gSaveBlock2.frontier.towerInterview.opponentSpecies = GetMonData(&gEnemyParty[gBattlerPartyIndexes[1]], MON_DATA_SPECIES);
    gSaveBlock2.frontier.towerInterview.playerSpecies = GetMonData(&gPlayerParty[gBattlerPartyIndexes[0]], MON_DATA_SPECIES);
...
static void SetNextBattleTentOpponent(void)
{
    ...
    SetBattleFacilityTrainerGfxId(gTrainerBattleOpponent_A, 0);
    if (gSaveBlock2.frontier.curChallengeBattleNum + 1 < 3)
       gSaveBlock2.frontier.trainerIds[gSaveBlock2.frontier.curChallengeBattleNum] = gTrainerBattleOpponent_A;
}
