...
int IsStringLengthAtLeast(const u8 *str, u32 n)
{
    u32 i;
...
#define NUM_STR(n)	( n>=10 ? CHAR_QUESTION_MARK : sDigits[n] )
#define HEX_STR(n)	( n>=16 ? CHAR_QUESTION_MARK : sDigits[n] )
...
u8 *ConvertUIntToDecimalStringN(u8 *dest, u32 value, enum StringConvertMode mode, u32 n)
{
    enum { WAITING_FOR_NONZERO_DIGIT, WRITING_DIGITS, WRITING_SPACES } state;
    s32 powerOfTen;
    s32 largestPowerOfTen = sPowersOfTen[n - 1];
...
u8 *ConvertIntToHexStringN(u8 *dest, s32 value, enum StringConvertMode mode, u32 n)
{
    enum { WAITING_FOR_NONZERO_DIGIT, WRITING_DIGITS, WRITING_SPACES } state;
    u8 i;
    s32 powerOfSixteen;
    ...
    {
        u8 c;
        s32 digit = value / powerOfSixteen;
...
bool32 IsStringNJapanese(u8 *str, u32 n)
{
    u32 i;
...
u32 GetExtCtrlCodeLength(u8 code)
{
