...
override CFLAGS += -mthumb -mthumb-interwork -O2 -mabi=aapcs -march=armv4t -mtune=arm7tdmi -mcpu=arm7tdmi -fno-toplevel-reorder -Wno-pointer-to-int-cast -Wno-pointer-to-int-cast -fallow-store-data-races -msoft-float -flto -ffat-lto-objects -fuse-linker-plugin
ROM := $(MODERN_ROM_NAME)
...
LDFLAGS = -Map ../../$(MAP) -no-pie -flto -ffat-lto-objects --relax --plugin=/opt/local/libexec/gcc/arm-none-eabi/11.2.0/liblto_plugin.so -plugin-opt=/opt/local/libexec/gcc/arm-none-eabi/11.2.0/lto-wrapper -plugin-opt=-fresolution=%u.res
...
$(C_BUILDDIR)/librfu_intr.o: CFLAGS := -mthumb-interwork -O2 -mabi=aapcs -march=armv4t -mtune=arm7tdmi -mcpu=arm7tdmi -fno-toplevel-reorder -Wno-pointer-to-int-cast -fallow-store-data-races -msoft-float  -flto -ffat-lto-objects -fuse-linker-plugin
endif
